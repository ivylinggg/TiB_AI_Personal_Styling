...
class ColourReportService {
  ...
  static void _drawColourAndFace(PdfPage page, ColourAnalysisResult result, SeasonColourProfile profile, PdfColor accent, PdfColor dark, PdfColor muted, PdfColor soft, PdfColor border, Uint8List? photoBytes) {
    ...
    _text(page, profile.description, PdfStandardFont(PdfFontFamily.helvetica, 9.7), muted, ui.Rect.fromLTWH(28, 304, 506, 32));

    _sectionTitle(page, 'Face shape', 355, accent, dark);
    if (photoBytes != null) {
      page.graphics.drawImage(PdfBitmap(photoBytes), ui.Rect.fromLTWH(28, 393, 156, 195));
      page.graphics.drawRectangle(pen: PdfPen(accent, width: 1.1), bounds: ui.Rect.fromLTWH(28, 393, 156, 195));
    }
    page.graphics.drawRectangle(brush: PdfSolidBrush(soft), pen: PdfPen(border), bounds: ui.Rect.fromLTWH(200, 393, 334, 195));
    _text(page, result.faceShape.trim().isEmpty ? 'Unknown' : result.faceShape, PdfStandardFont(PdfFontFamily.helvetica, 25, style: PdfFontStyle.bold), dark, ui.Rect.fromLTWH(220, 414, 270, 32));
    _text(page, result.faceShapeDescription.trim().isEmpty ? 'Proportion-based analysis from your face contour.' : result.faceShapeDescription, PdfStandardFont(PdfFontFamily.helvetica, 9.2), muted, ui.Rect.fromLTWH(220, 456, 285, 48));
    final m = result.faceMeasurements;
    _smallMeasure(page, 'Length / cheekbone', m['faceLengthToCheekbone'], 515, accent, dark, muted);
    _smallMeasure(page, 'Forehead / cheekbone', m['foreheadToCheekbone'], 540, accent, dark, muted);
    _smallMeasure(page, 'Jaw / cheekbone', m['jawToCheekbone'], 565, accent, dark, muted);
  }
