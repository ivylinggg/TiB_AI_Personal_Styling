...
class ColourReportService {
  ...
  static Future<Uint8List> generateBytes({required ColourAnalysisResult result}) async {
    ...
    try {
      ...
      _drawReference(document.pages.add(), result, accent, dark, muted, soft, border);
  ...
  static void _drawCover(PdfPage page, ColourAnalysisResult result, SeasonColourProfile profile, PdfColor accent, PdfColor dark, PdfColor muted, PdfColor soft, PdfColor border, Uint8List? photoBytes, String date) {
    ...
    _text(page, result.season.toUpperCase(), PdfStandardFont(PdfFontFamily.helvetica, 39, style: PdfFontStyle.bold), accent, ui.Rect.fromLTWH(48, 158, 430, 48));
    ...
    _text(page, result.season.toUpperCase(), PdfStandardFont(PdfFontFamily.helvetica, 24, style: PdfFontStyle.bold), accent, ui.Rect.fromLTWH(312, 318, 180, 30));
    _text(page, profile.dimension, PdfStandardFont(PdfFontFamily.helvetica, 10, style: PdfFontStyle.bold), dark, ui.Rect.fromLTWH(312, 351, 180, 28));
    _text(page, result.faceShape.trim().isEmpty ? 'Unknown face shape' : '${result.faceShape} face shape', PdfStandardFont(PdfFontFamily.helvetica, 13, style: PdfFontStyle.bold), dark, ui.Rect.fromLTWH(312, 389, 170, 34));
    _smallProfile(page, 'Undertone', result.undertone, 433, dark, muted);
    _smallProfile(page, 'Depth', result.brightness, 472, dark, muted);
    _smallProfile(page, 'Contrast', result.contrast, 511, dark, muted);
    _smallProfile(page, 'Confidence', '${(result.confidence * 100).round()}%', 548, dark, muted);
  ...
  static void _drawColourAndFace(PdfPage page, ColourAnalysisResult result, SeasonColourProfile profile, PdfColor accent, PdfColor dark, PdfColor muted, PdfColor soft, PdfColor border, Uint8List? photoBytes) {
    _header(page, '01 · COLOUR + FACE ANALYSIS', 'Your personal colour result and face proportions', accent, dark, muted);

    _sectionTitle(page, 'PERSONAL COLOUR RESULT', 112, accent, dark);
    _text(page, result.season.toUpperCase(), PdfStandardFont(PdfFontFamily.helvetica, 30, style: PdfFontStyle.bold), accent, ui.Rect.fromLTWH(28, 148, 210, 38));
    _text(page, profile.dimension, PdfStandardFont(PdfFontFamily.helvetica, 11.5, style: PdfFontStyle.bold), dark, ui.Rect.fromLTWH(28, 190, 300, 20));
    _metricCard(page, 28, 226, 158, 'UNDERTONE', result.undertone, accent, dark, soft, border, muted);
    _metricCard(page, 202, 226, 158, 'DEPTH', result.brightness, accent, dark, soft, border, muted);
    _metricCard(page, 376, 226, 158, 'CONTRAST', result.contrast, accent, dark, soft, border, muted);
    _metricCard(page, 28, 300, 158, 'CHROMA', result.chroma, accent, dark, soft, border, muted);
    _metricCard(page, 202, 300, 158, 'CLARITY', result.clarity, accent, dark, soft, border, muted);
    _metricCard(page, 376, 300, 158, 'CONFIDENCE', '${(result.confidence * 100).round()}%', accent, dark, soft, border, muted);
    _text(page, profile.description, PdfStandardFont(PdfFontFamily.helvetica, 9.7), muted, ui.Rect.fromLTWH(28, 378, 506, 32));

    _sectionTitle(page, 'Face shape', 425, accent, dark);
    if (photoBytes != null) {
      page.graphics.drawImage(PdfBitmap(photoBytes), ui.Rect.fromLTWH(28, 463, 156, 195));
      page.graphics.drawRectangle(pen: PdfPen(accent, width: 1.1), bounds: ui.Rect.fromLTWH(28, 463, 156, 195));
    }
    page.graphics.drawRectangle(brush: PdfSolidBrush(soft), pen: PdfPen(border), bounds: ui.Rect.fromLTWH(200, 463, 334, 195));
    _text(page, result.faceShape.trim().isEmpty ? 'Unknown' : result.faceShape, PdfStandardFont(PdfFontFamily.helvetica, 25, style: PdfFontStyle.bold), dark, ui.Rect.fromLTWH(220, 484, 270, 32));
    _text(page, result.faceShapeDescription.trim().isEmpty ? 'Proportion-based analysis from your face contour.' : result.faceShapeDescription, PdfStandardFont(PdfFontFamily.helvetica, 9.2), muted, ui.Rect.fromLTWH(220, 526, 285, 48));
    final m = result.faceMeasurements;
    _smallMeasure(page, 'Length / cheekbone', m['faceLengthToCheekbone'], 585, accent, dark, muted);
    _smallMeasure(page, 'Forehead / cheekbone', m['foreheadToCheekbone'], 610, accent, dark, muted);
    _smallMeasure(page, 'Jaw / cheekbone', m['jawToCheekbone'], 635, accent, dark, muted);
  }
  ...
  static void _drawPaletteAndStyle(PdfPage page, ColourAnalysisResult result, SeasonColourProfile profile, PdfColor accent, PdfColor dark, PdfColor muted, PdfColor soft, PdfColor border) {
    _header(page, '02 · YOUR SEASON COLOUR PALETTE', 'The colours that support your personal result', accent, dark, muted);

    _sectionTitle(page, 'BEST COLOURS FOR YOU', 112, accent, dark);
    _drawSwatches(page, profile.bestColours.take(16).toList(), 154, accent, dark, border);

    _sectionTitle(page, 'Beauty colours', 400, accent, dark);
    _drawBeauty(page, profile.eyeShadowColours.take(6).toList(), 'Eye shadow', 438, accent, dark, border);
    _drawBeauty(page, profile.blushColours.take(6).toList(), 'Blush', 560, accent, dark, border);

    _sectionTitle(page, 'Style identity', 650, accent, dark);
    _drawPills(page, profile.keywords.take(8).toList(), 688, accent, dark, soft, border);
    _text(page, _styleSummary(result, profile), PdfStandardFont(PdfFontFamily.helvetica, 8.7), muted, ui.Rect.fromLTWH(28, 725, 506, 30));
  }

  static void _drawReference(PdfPage page, ColourAnalysisResult result, PdfColor accent, PdfColor dark, PdfColor muted, PdfColor soft, PdfColor border) {
    final profile = SeasonColourGuide.forSeason(result.season);
    _header(page, '03 · SEASON REFERENCE', 'Your result highlighted against all four seasons', accent, dark, muted);
    ...
    for (var i = 0; i < profiles.length && i < positions.length; i++) {
      final seasonProfile = profiles[i];
      final rect = positions[i];
      final color = _seasonColor(seasonProfile.name);
      final isSelected = seasonProfile.name.toLowerCase() == result.season.toLowerCase();
      page.graphics.drawRectangle(
        brush: PdfSolidBrush(isSelected ? PdfColor(255, 250, 245) : soft),
        pen: PdfPen(isSelected ? accent : color, width: isSelected ? 2.5 : 1.0),
        bounds: rect,
      );
      if (isSelected) {
        _text(page, 'YOUR RESULT', PdfStandardFont(PdfFontFamily.helvetica, 7.5, style: PdfFontStyle.bold), accent, ui.Rect.fromLTWH(rect.left + rect.width - 82, rect.top + 14, 68, 12), alignRight: true);
      }
      _text(page, seasonProfile.name.toUpperCase(), PdfStandardFont(PdfFontFamily.helvetica, 13, style: PdfFontStyle.bold), color, ui.Rect.fromLTWH(rect.left + 14, rect.top + 14, 200, 18));
      _text(page, seasonProfile.dimension, PdfStandardFont(PdfFontFamily.helvetica, 9.5, style: PdfFontStyle.bold), dark, ui.Rect.fromLTWH(rect.left + 14, rect.top + 40, 200, 16));
      _text(page, seasonProfile.description, PdfStandardFont(PdfFontFamily.helvetica, 8), muted, ui.Rect.fromLTWH(rect.left + 14, rect.top + 64, 220, 42));
      _drawSwatchesCompact(page, seasonProfile.bestColours.take(6).toList(), rect.left + 14, rect.top + 118, rect.width - 28, color, dark);
    }

    _text(page, 'Your personal result: ${result.season.toUpperCase()} · ${profile.dimension}', PdfStandardFont(PdfFontFamily.helvetica, 10, style: PdfFontStyle.bold), accent, ui.Rect.fromLTWH(28, 722, 506, 16), alignCenter: true);
  }
  ...
  static String _styleSummary(ColourAnalysisResult result, SeasonColourProfile profile) {
    ...
    return 'Your strongest direction combines ${profile.dimension.toLowerCase()} colour harmony with $face. Analysis confidence is $confidence%. Use the palette as your core wardrobe language, then adjust silhouette and details for occasion and personal preference.';
  }
