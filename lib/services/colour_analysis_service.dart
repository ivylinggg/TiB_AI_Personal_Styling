...
class ColourAnalysisService {
  ...
  static Future<ColourAnalysisResult> analyse({
    ...
    if (decoded == null) throw const FormatException('Unable to read the selected image.');

    final source = img.bakeOrientation(decoded);
    final sample = _samplePortrait(source);
    if (!sample.isUsable) {
      throw const FormatException('Unable to reliably analyse this photo. Please use one clear, front-facing portrait in even natural light.');
    }
    ...
    final season = _season(undertone: undertone, brightness: brightness, contrast: contrast, chroma: chroma, clarity: clarity);
  ...
  static _PortraitSample _samplePortrait(img.Image image) {
    ...
    var redGreenSum = 0.0;
    var greenBlueSum = 0.0;

    for (var y = top; y < bottom; y += 3) {
      for (var x = left; x < right; x += 3) {
        final p = image.getPixel(x, y);
        final rgb = _ColourPixel(r: p.r.toDouble(), g: p.g.toDouble(), b: p.b.toDouble());
        if (!_looksLikeSkin(rgb.r, rgb.g, rgb.b)) continue;
        skinPixels.add(rgb);
        redGreenSum += rgb.r - rgb.g;
        greenBlueSum += rgb.g - rgb.b;
      }
    }
    ...
    final average = skinPixels.reduce((a, b) => _ColourPixel(r: a.r + b.r, g: a.g + b.g, b: a.b + b.b));
    final avg = _ColourPixel(r: average.r / count, g: average.g / count, b: average.b / count);

    final luminances = skinPixels.map((p) => _luminance(p.r, p.g, p.b)).toList()..sort();
    ...
    for (var y = 0; y < height; y += 10) {
      for (var x = 0; x < width; x += 10) {
        ...
        if (x >= left && x <= right && y >= top && y <= bottom) {
          if (value < p20 * .82) { facialDark += value; facialDarkCount++; }
          if (value > p80 * 1.08) { facialLight += value; facialLightCount++; }
          if (_looksLikeSkin(r, g, b)) {
            final warmth = r - b;
            if (warmth > 48) warmRegion += warmth;
            if (warmth < 28) coolRegion += 28 - warmth;
          }
        }
      }
    }
    ...
    final skinContrast = math.max(0.0, math.max(p80 - darkReference, lightReference - p20));
    final averageWarmth = ((redGreenSum / count) * .62) + ((greenBlueSum / count) * .18) + ((warmRegion - coolRegion) / math.max(1.0, count) * .20);
    final averageChroma = skinPixels.map((p) => math.max(p.r, math.max(p.g, p.b)) - math.min(p.r, math.min(p.g, p.b))).reduce((a, b) => a + b) / count;
    final claritySignal = (averageChroma / math.max(p50, 1)) * 100.0;
    final neutralitySignal = ((avg.r - avg.g).abs() + (avg.g - avg.b).abs()) / math.max(avg.r + avg.g + avg.b, 1) * 100.0;
  ...
  static double _luminance(double r, double g, double b) {
    ...
    if (maxChannel <= 1.0) return (0.2126 * r + 0.7152 * g + 0.0722 * b) * 255.0;
  ...
  static bool _looksLikeSkin(double r, double g, double b) {
    ...
    final nr = r / sum, ng = g / sum, nb = b / sum;
    final hueLike = nr > .27 && nr < .58 && ng > .20 && ng < .44 && nb < .36;
    return hueLike && r >= g * .92 && g >= b * .92;
  }
  ...
  static String _clarity(_PortraitSample sample) {
    if (sample.claritySignal >= 11 && sample.skinSpread >= 20) return 'Clear';
  ...
  static String _season({required String undertone, required String brightness, required String contrast, required String chroma, required String clarity}) {
    final highChroma = chroma == 'High';
    final soft = clarity == 'Soft';
    final clear = clarity == 'Clear';
    if (undertone == 'Warm') {
      if ((brightness == 'Light' || brightness == 'Medium') && highChroma && clear) return 'Spring';
      if (brightness == 'Deep' || brightness == 'Medium-Deep' || soft) return 'Autumn';
      return contrast == 'High' && highChroma ? 'Spring' : 'Autumn';
    }
    if (undertone == 'Cool') {
      if (brightness == 'Light' && soft) return 'Summer';
      if (brightness == 'Deep' || brightness == 'Medium-Deep' || (clear && highChroma)) return 'Winter';
      return contrast == 'Low' ? 'Summer' : 'Winter';
    }
    if (brightness == 'Light') return soft ? 'Summer' : 'Spring';
    if (brightness == 'Deep' || brightness == 'Medium-Deep') return clear && contrast == 'High' ? 'Winter' : 'Autumn';
...
class _PortraitSample {
  final double skinToneValue, skinMeanLuminance, skinSpread, skinContrast, averageWarmth, averageChroma, claritySignal, neutralitySignal, globalRange;
  final int skinCount;
  const _PortraitSample({required this.skinToneValue, required this.skinMeanLuminance, required this.skinSpread, required this.skinContrast, required this.averageWarmth, required this.averageChroma, required this.claritySignal, required this.neutralitySignal, required this.globalRange, required this.skinCount});
  factory _PortraitSample.empty() => const _PortraitSample(skinToneValue: 0, skinMeanLuminance: 0, skinSpread: 0, skinContrast: 0, averageWarmth: 0, averageChroma: 0, claritySignal: 0, neutralitySignal: 0, globalRange: 0, skinCount: 0);
