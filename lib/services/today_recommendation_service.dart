...
class TodayRecommendation {
  ...
  factory TodayRecommendation.fromJson(
    ...
    return TodayRecommendation(
      ...
      reason: read('whyItWorks', 'This recommendation is tailored to your VYEA profile.'),
...
class TodayRecommendationService {
  ...
  static Future<TodayRecommendation> getRecommendation({
    ...
    final fallback = build(analysis: analysis, personalStyle: personalStyle);
  ...
  static TodayRecommendation build({ColourAnalysisResult? analysis, String? personalStyle}) {
    ...
    final fallbackStyles = <TodayRecommendation>[
      ...
      TodayRecommendation(style: styleHint.isEmpty ? 'Modern Feminine' : styleHint, tags: const ['Soft', 'Stylish', 'Polished'], colour: '—', outfit: 'A softly fitted top with a clean skirt or tailored trousers.', reason: 'A soft silhouette with structure creates an easy modern feminine balance.'),
      TodayRecommendation(style: styleHint.isEmpty ? 'Casual Glow' : styleHint, tags: const ['Relaxed', 'Bright', 'Fun'], colour: '—', outfit: 'A relaxed outfit with one brighter accent or accessory.', reason: 'One playful detail keeps a casual look intentional and fresh.'),
      TodayRecommendation(style: styleHint.isEmpty ? 'Effortless Weekend' : styleHint, tags: const ['Casual', 'Easy', 'Cool'], colour: '—', outfit: 'An easy top, relaxed bottoms and comfortable statement shoes.', reason: 'Comfort-first styling can still look considered when proportions stay clean.'),
  ...
  static String _todayColour(ColourAnalysisResult? result) {
    ...
    final colours = result.colours.map((colour) => colour.trim()).where((colour) => colour.isNotEmpty).toList();
