...
class LiveConsultancyService {
  ...
  static Stream<QuerySnapshot<Map<String, dynamic>>> conversationMessages(String uid) =>
      _messages(uid).orderBy('createdAt').snapshots();
  ...
  static Future<void> assignConsultant({
    ...
    await ref.set({
      ...
      'assignedConsultantName': consultantName.trim().isEmpty ? 'TiB Consultant' : consultantName.trim(),
      'status': 'assigned',
      'assignedAt': FieldValue.serverTimestamp(),
      'updatedAt': FieldValue.serverTimestamp(),
    }, SetOptions(merge: true));
