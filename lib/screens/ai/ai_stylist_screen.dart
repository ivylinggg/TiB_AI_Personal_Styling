...
class _AIStylistScreenState extends State<AIStylistScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: _openPreferences, icon: const Icon(Icons.tune_rounded), tooltip: 'My Style'),
          IconButton(onPressed: _openWardrobe, icon: const Icon(Icons.checkroom_outlined), tooltip: 'My Wardrobe'),
        ],
      ),
      body: SafeArea(
        child: RefreshIndicator(
          ...
          child: ListView(
            ...
            children: [
              ...
              _title('What are you dressing for?', 'Tell me the moment. I will help with the rest.'),
              const SizedBox(height: 12),
              _occasions(),
              const SizedBox(height: 18),
              _recommendation(result != null, mainColour, matches),
              const SizedBox(height: 26),
              _title('A little help, when you need it', 'Small decisions can make getting dressed easier.'),
              const SizedBox(height: 12),
              _helpGrid(),
    ...
    return Container(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Let’s make getting dressed easier.', style: TextStyle(color: _text, fontSize: 21, fontWeight: FontWeight.w700)),
          const SizedBox(height: 7),
          Text(season == null ? 'Start with your colour analysis, then tell me what feels like you.' : 'I know your $season profile and ${undertone ?? 'colour'} undertone. Now let’s make it personal.', style: const TextStyle(color: _muted, fontSize: 14, height: 1.45)),
        ])),
      ]),
    );
  }
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(season == null ? 'Complete your colour analysis' : '$season · ${undertone ?? 'Unknown'} undertone', style: const TextStyle(color: _text, fontWeight: FontWeight.w700)),
        ])),
        if (colours.isNotEmpty) Row(children: colours.take(3).map((c) => Container(width: 21, height: 21, margin: const EdgeInsets.only(left: 5), decoration: BoxDecoration(color: _colour(c), shape: BoxShape.circle))).toList()),
      ]),
    );
  }

  Widget _personalCard() {
    final wardrobeText = _loading ? 'Looking through your wardrobe...' : _wardrobe.isEmpty ? 'Add a few pieces I can style for you' : '${_wardrobe.length} pieces ready to style';
    final styleText = _loading ? 'Loading your style...' : _styles.isEmpty ? 'Tell me what feels like you' : _styles.take(2).join(' · ');
    return Column(children: [
      _action(Icons.checkroom_outlined, 'My wardrobe', wardrobeText, _openWardrobe),
      const SizedBox(height: 8),
      _action(Icons.favorite_border_rounded, 'My style', styleText, _openPreferences),
    ]);
  }
  ...
  Widget _action(IconData icon, String title, String subtitle, VoidCallback onTap) {
    return Material(color: Colors.white, borderRadius: BorderRadius.circular(17), child: InkWell(onTap: onTap, borderRadius: BorderRadius.circular(17), child: Padding(padding: const EdgeInsets.all(14), child: Row(children: [
      Container(width: 40, height: 40, decoration: const BoxDecoration(color: _soft, shape: BoxShape.circle), child: Icon(icon, color: _brown, size: 20)),
      const SizedBox(width: 12),
  ...
  Widget _title(String title, String subtitle) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(color: _text, fontSize: 20, fontWeight: FontWeight.w700)), const SizedBox(height: 4), Text(subtitle, style: const TextStyle(color: _muted, fontSize: 13, height: 1.4))]);

  Widget _occasions() {
    const values = ['Everyday', 'Work', 'Date', 'Event', 'Weekend'];
    return SizedBox(height: 44, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: values.length, separatorBuilder: (_, _) => const SizedBox(width: 8), itemBuilder: (_, index) {
      final selected = values[index] == _occasion;
      return ChoiceChip(label: Text(values[index]), selected: selected, onSelected: (_) => setState(() => _occasion = values[index]), selectedColor: _brown, backgroundColor: Colors.white, labelStyle: TextStyle(color: selected ? Colors.white : _text, fontWeight: FontWeight.w600));
    }));
  }
  ...
  List<WardrobeItem> _matchingItems(List<String> colours) {
    ...
    final matches = _wardrobe.where((item) => wanted.any((c) => c.contains(item.colour.toLowerCase()) || item.colour.toLowerCase().contains(c))).toList();
    final rest = _wardrobe.where((item) => !matches.contains(item)).toList();
    return [...matches, ...rest].take(4).toList();
  }

  Widget _recommendation(bool hasProfile, String mainColour, List<WardrobeItem> items) {
    WardrobeItem? top;
    WardrobeItem? bottom;
    WardrobeItem? shoes;
    for (final item in items) {
      if (top == null && (item.category == 'Tops' || item.category == 'Dresses')) top = item;
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24), boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: .04), blurRadius: 16, offset: const Offset(0, 6))]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: _soft, borderRadius: BorderRadius.circular(20)), child: Text(_occasion, style: const TextStyle(color: _brown, fontSize: 12, fontWeight: FontWeight.w700))), const Spacer(), const Icon(Icons.favorite_border_rounded, color: _brown)]),
        ...
        const SizedBox(height: 7),
        Text(items.isEmpty ? (hasProfile ? 'Try $mainColour as your main colour, then add a calm neutral.' : 'Complete your colour analysis and add a few wardrobe pieces so I can style what you actually own.') : 'I found pieces you already own that can work together for $_occasion.', style: const TextStyle(color: _muted, fontSize: 14, height: 1.5)),
        const SizedBox(height: 17),
        if (top != null) _lookRow(Icons.checkroom_outlined, 'Main piece', top.name, top.colour),
        if (bottom != null) ...[const SizedBox(height: 11), _lookRow(Icons.layers_outlined, 'Bottom', bottom.name, bottom.colour)],
        if (shoes != null) ...[const SizedBox(height: 11), _lookRow(Icons.directions_walk_outlined, 'Shoes', shoes.name, shoes.colour)],
        ...
        const SizedBox(height: 18),
        Row(children: [
          Expanded(child: ElevatedButton.icon(onPressed: () => _showRecommendation(items, mainColour), icon: const Icon(Icons.auto_awesome_rounded), label: const Text('Build this look'), style: ElevatedButton.styleFrom(backgroundColor: _brown, foregroundColor: Colors.white, minimumSize: const Size.fromHeight(50), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15))))),
          const SizedBox(width: 10),
          IconButton.filledTonal(onPressed: _openWardrobe, icon: const Icon(Icons.checkroom_outlined)),
        ]),
      ]),
    );
  }

  Widget _lookRow(IconData icon, String title, String name, String colour) => Row(children: [Container(width: 38, height: 38, decoration: const BoxDecoration(color: _soft, shape: BoxShape.circle), child: Icon(icon, size: 20, color: _brown)), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(color: _muted, fontSize: 11)), const SizedBox(height: 2), Text(name, style: const TextStyle(color: _text, fontWeight: FontWeight.w600)), if (colour.isNotEmpty) Text(colour, style: const TextStyle(color: _muted, fontSize: 11))]))]);
  ...
  Widget _helpGrid() => GridView.count(crossAxisCount: 2, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: 1.12, children: [
    _help(Icons.checkroom_outlined, 'My wardrobe', 'Style pieces you already own.', _openWardrobe),
    _help(Icons.palette_outlined, 'Mix & match', 'Find colours that work together.', _showColourIdeas),
    _help(Icons.shopping_bag_outlined, 'Shopping help', 'See what your wardrobe is missing.', _showShoppingIdeas),
    _help(Icons.event_outlined, 'Dress for it', 'Make an occasion feel effortless.', _showOccasionIdeas),
  ]);

  Widget _help(IconData icon, String title, String description, VoidCallback onTap) => Material(color: Colors.white, borderRadius: BorderRadius.circular(20), child: InkWell(onTap: onTap, borderRadius: BorderRadius.circular(20), child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Container(width: 42, height: 42, decoration: const BoxDecoration(color: _soft, shape: BoxShape.circle), child: Icon(icon, color: _brown)), const Spacer(), Text(title, style: const TextStyle(color: _text, fontWeight: FontWeight.w700)), const SizedBox(height: 4), Text(description, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _muted, fontSize: 12, height: 1.35))]))));

  Widget _humanNote() => Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: const Color(0xFFF2EEE9), borderRadius: BorderRadius.circular(20)), child: const Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(Icons.tips_and_updates_outlined, color: _brown), SizedBox(width: 12), Expanded(child: Text('Good style does not mean following every trend. It is about finding choices that feel comfortable, confident and right for you.', style: TextStyle(color: _muted, fontSize: 13, height: 1.5)))]));

  void _showRecommendation(List<WardrobeItem> items, String colour) {
    _sheet('Your personal look', Icons.auto_awesome_rounded, [
      _sheetText(items.isEmpty ? 'Add a few pieces first. Then I can build outfits from what you actually own.' : 'I picked from your own wardrobe first, so this recommendation is realistic for you.'),
      ...items.map((item) => _sheetItem(item.category, '${item.name} · ${item.colour}')).take(4),
  ...
  void _showShoppingIdeas() {
    ...
    _sheet('Shopping assistant', Icons.shopping_bag_outlined, [
      _sheetText(missing.isEmpty ? 'Your basics are covered. Look for variety rather than duplicates.' : 'These are the wardrobe gaps I would fill first.'),
      ...missing.map((m) => _sheetItem('Consider', m)),
      _sheetItem('Before buying', 'Ask yourself if the new piece works with at least three things you already own.'),
    ]);
  }
  ...
  void _sheet(String title, IconData icon, List<Widget> children) {
    showModalBottomSheet<void>(context: context, showDragHandle: true, backgroundColor: _cream, builder: (context) => SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(20, 5, 20, 28), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Container(width: 44, height: 44, decoration: const BoxDecoration(color: _soft, shape: BoxShape.circle), child: Icon(icon, color: _brown)), const SizedBox(width: 12), Expanded(child: Text(title, style: const TextStyle(color: _text, fontSize: 21, fontWeight: FontWeight.w700)))]), const SizedBox(height: 20), ...children]))));
  }
  ...
  Color _colour(String name) {
    ...
    if (v.contains('pink') || v.contains('rose')) return const Color(0xFFE8A7B7);
    if (v.contains('red') || v.contains('coral')) return const Color(0xFFD97968);
    if (v.contains('orange') || v.contains('peach')) return const Color(0xFFE7A16F);
    if (v.contains('yellow') || v.contains('gold')) return const Color(0xFFD8B85A);
    if (v.contains('green') || v.contains('olive')) return const Color(0xFF8A9A68);
    if (v.contains('blue') || v.contains('navy')) return const Color(0xFF7189A8);
    if (v.contains('purple') || v.contains('violet')) return const Color(0xFF9A7AA8);
    if (v.contains('brown') || v.contains('beige') || v.contains('neutral')) return const Color(0xFFB59A83);
