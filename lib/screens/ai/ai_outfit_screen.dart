...
class _AIOutfitScreenState extends State<AIOutfitScreen> with WidgetsBindingObserver {
  ...
  List<WardrobeItem> _fallbackLook(ColourAnalysisResult profile) {
    ...
    WardrobeItem? pick(String category) {
    ...
    WardrobeItem? pickLower() {
      for (final item in sorted) {
        if ((item.category == 'Bottoms' || item.category == 'Skirts') && !used.contains(item.id)) return item;
      }
      return null;
    }

    final hasTop = sorted.any((item) => item.category == 'Tops');
    final hasLower = sorted.any((item) => item.category == 'Bottoms' || item.category == 'Skirts');
    final dresses = sorted.where((item) => item.category == 'Dresses');
    final suits = sorted.where((item) => item.category == 'Suits');

    if (hasTop && hasLower) {
      final top = pick('Tops');
      final lower = pickLower();
    ...
    if (result.isNotEmpty && (baseHasTop || baseHasOnePiece)) {
      final jacket = pick('Jackets');
      if (jacket != null) {
        result.add(jacket);
        used.add(jacket.id);
      }
    ...
    if (shoes != null && result.length < 4) {
      result.add(shoes);
      used.add(shoes.id);
    }
    final accessory = pick('Accessories');
    if (accessory != null && result.length < 4) result.add(accessory);
  ...
  Future<void> _generate(ColourAnalysisResult profile) async {
    ...
    try {
      ...
      if (aiResult != null) {
        ...
        final ids = [aiResult.topId, aiResult.bottomId, aiResult.shoesId, aiResult.accessoryId];
        for (final id in ids) {
          final item = _findWardrobeItem(id);
          if (item != null) aiLook.add(item);
        }
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 10.5, fontWeight: FontWeight.w800)), Text(label, style: const TextStyle(color: Colors.white54, fontSize: 8.5))])),
          ],
        ),
      ),
    );
    ...
    return Column(
      ...
      children: [
        Row(children: [
          ...
          if (_aiResult != null) Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5), decoration: BoxDecoration(color: AppColors.peach.withValues(alpha: .38), borderRadius: BorderRadius.circular(9)), child: const Text('AI MATCHED', style: TextStyle(fontSize: 7.5, fontWeight: FontWeight.w900, letterSpacing: .8))),
        ]),
