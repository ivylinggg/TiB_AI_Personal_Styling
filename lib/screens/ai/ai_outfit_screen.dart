...
class _AIOutfitScreenState extends State<AIOutfitScreen> {
  ...
  Future<void> _feedback(WardrobeItem item, bool liked) async {
    if (item.id.isEmpty || _feedbackSaving) return;
    setState(() {
      if (liked) {
  ...
  Future<void> _restyle({String? actionCategory}) async {
    final uid = FirebaseAuth.instance.currentUser?.uid.trim();
    final profile = context.read<AnalysisProvider>().result;
    if (uid == null || uid.isEmpty || profile == null || _styling || _wardrobe.isEmpty) return;

    final currentIds = _look.map((item) => item.id).where((id) => id.isNotEmpty).toSet();
    if (actionCategory != null) {
      final replacement = _pickAlternative(actionCategory, currentIds);
      if (replacement != null) {
        final replaced = _look.where((item) => item.category != actionCategory).toList();
        replaced.add(replacement);
        final key = _lookKey(replaced);
        if (key != null) _excludedLookKeys.add(key);
      }
    } else {
      final key = _lookKey(_look);
  ...
  WardrobeItem? _pickAlternative(String category, Set<String> currentIds) {
    final candidates = _wardrobe.where((item) => item.category == category && !currentIds.contains(item.id)).toList(growable: false);
    if (candidates.isEmpty) return null;
    final loved = candidates.where((item) => _lovedItemIds.contains(item.id)).toList(growable: false);
    if (loved.isNotEmpty) return loved.first;
    final favourites = candidates.where((item) => item.isFavourite).toList(growable: false);
    if (favourites.isNotEmpty) return favourites.first;
    return candidates.first;
  }

  Future<void> _changeShoes() => _restyle(actionCategory: 'Shoes');
  Future<void> _restyleLook() => _restyle();
  ...
  Widget _result(ColourAnalysisResult? profile) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        _matchBadge(result?.matchScore ?? 0),
      ]),
      ...
      GridView.builder(
        ...
        gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 11, crossAxisSpacing: 11, childAspectRatio: 0.82),
  ...
  Widget _actionGrid(ColourAnalysisResult profile) {
    final actions = <({String label, IconData icon, VoidCallback onTap})>[
      (label: _savedLook ? 'Saved' : 'Save look', icon: _savedLook ? Icons.bookmark_rounded : Icons.bookmark_border_rounded, onTap: _savingLook ? () {} : () => _saveLook(profile)),
      (label: 'Try another', icon: Icons.auto_awesome_outlined, onTap: _styling ? () {} : () => _tryAnother(profile)),
      (label: 'Restyle', icon: Icons.refresh_rounded, onTap: _styling ? () {} : _restyleLook),
      (label: 'Change shoes', icon: Icons.sports_rounded, onTap: _styling ? () {} : _changeShoes),
    ];
    return GridView.builder(
      ...
      itemBuilder: (_, index) {
        ...
        return action.onTap == null ? const SizedBox.shrink() : OutlinedButton.icon(
          onPressed: _styling ? null : action.onTap,
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Expanded(child: Stack(fit: StackFit.expand, children: [
          ...
          Positioned(top: 5, right: 5, child: Row(children: [
        ...
        Padding(padding: const EdgeInsets.fromLTRB(11, 9, 11, 11), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget _feedbackButton(IconData icon, bool active, VoidCallback onPressed) => Material(
    ...
    child: InkWell(onTap: _feedbackSaving ? null : onPressed, borderRadius: BorderRadius.circular(9), child: Padding(padding: const EdgeInsets.all(6), child: Icon(icon, size: 13, color: active ? Colors.white : Colors.white70))),
  );
