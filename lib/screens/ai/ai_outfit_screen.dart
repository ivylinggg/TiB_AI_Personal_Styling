...
class _AIOutfitScreenState extends State<AIOutfitScreen> {
  ...
  Future<void> _restyleLook(ColourAnalysisResult profile) async {
    final currentKey = _lookKey(_look);
    if (currentKey != null) _excludedLookKeys.add(currentKey);
    await _generate(profile);
  }

  Future<void> _changeShoes(ColourAnalysisResult profile) async {
    if (_look.isEmpty) return;
    final currentKey = _lookKey(_look);
    if (currentKey != null) _excludedLookKeys.add(currentKey);
    final uid = FirebaseAuth.instance.currentUser?.uid.trim();
    if (uid == null || uid.isEmpty || uid != _loadedUid) return;
    ...
    try {
      ...
      final styles = List<String>.from(prefs?['styles'] ?? const []);
      final preferences = List<String>.from(prefs?['preferences'] ?? const []);
      for (final shoe in _wardrobe.where((item) => item.category == 'Shoes')) {
        final candidate = [..._look.where((item) => item.category != 'Shoes'), shoe];
        final result = AiStylingService.sanitizeLook(candidate,
          profile: profile,
          occasion: _occasion,
          styles: styles,
          preferences: preferences,
        ...
        if (result.isNotEmpty && key != null && !_excludedLookKeys.contains(key)) {
          setState(() {
            _look = result;
            _aiResult = _aiResult;
            _styling = false;
    ...
    if (!mounted) return;
    setState(() => _styling = false);
    _showFeedback('I could not find a better shoe pairing in your wardrobe.');
  }
  ...
  Future<void> _feedback(WardrobeItem item, bool liked) async {
    ...
    final ids = _look.map((entry) => entry.id).where((id) => id.isNotEmpty).toList();
    if (ids.isNotEmpty) {
  ...
  Widget _result(ColourAnalysisResult? profile) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(24), border: Border.all(color: AppColors.border)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800)), const SizedBox(height: 6), Text(explanation, style: const TextStyle(fontSize: 12.5, color: AppColors.textSecondary, height: 1.45)), if (result?.colourDirection != null) ...[const SizedBox(height: 13), Row(crossAxisAlignment: CrossAxisAlignment.start, children: [const Icon(Icons.palette_outlined, size: 17, color: AppColors.primary), const SizedBox(width: 8), Expanded(child: Text(result!.colourDirection!, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, height: 1.35)))])]])),
  ...
  Widget _actionGrid(ColourAnalysisResult profile) {
    final actions = <({String label, IconData icon, VoidCallback? onTap})>[
      ...
      (label: 'Restyle', icon: Icons.refresh_rounded, onTap: _styling ? null : () => _restyleLook(profile)),
      (label: 'Change shoes', icon: Icons.sports_rounded, onTap: _styling ? null : () => _changeShoes(profile)),
    ];
    return GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: actions.length, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 10, crossAxisSpacing: 10, childAspectRatio: 2.5), itemBuilder: (_, index) {
      final action = actions[index];
      return OutlinedButton.icon(onPressed: action.onTap, icon: Icon(action.icon, size: 17), label: Text(action.label, maxLines: 1, overflow: TextOverflow.ellipsis), style: OutlinedButton.styleFrom(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))));
    });
  }
