...
class _AIOutfitScreenState extends State<AIOutfitScreen> {
  ...
  Future<void> _generate(ColourAnalysisResult profile) async {
    ...
    if (fallback.isEmpty) {
      _showFeedback('I could not build a valid outfit from your current wardrobe.');
    }
  }
  ...
  Widget _hero(ColourAnalysisResult? profile) {
    final colourSummary = profile == null
        ? 'Add your Colour Analysis to make your outfits more personal.'
        : 'Built around ${profile.undertone} · ${profile.brightness} · ${profile.chroma} · ${profile.contrast}.';
  ...
  Widget _result(ColourAnalysisResult? profile) {
    ...
    final explanation = result?.explanation.isNotEmpty == true
        ? result!.explanation
        : 'A balanced match based on your personal colour profile, style and wardrobe.';
      ...
      children: [
        ...
        Row(
          children: [
            ...
            Expanded(
              child: FilledButton.icon(
                onPressed: _styling ? null : () => _tryAnother(profile),
    ...
    return Container(
      ...
      child: Column(
        children: [
          Text(
            '$score%',
            style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900, color: AppColors.success),
          ),
          const Text(
            'MATCH',
            style: TextStyle(fontSize: 8, fontWeight: FontWeight.w800, letterSpacing: 0.8, color: AppColors.success),
          ),
        ],
      ),
    );
              ...
              children: [
                if (item.imageUrl.isNotEmpty)
                  ...
                  Container(
                    ...
                    child: const Icon(Icons.checkroom_outlined, color: AppColors.primary, size: 36),
                  ),
                Positioned(
                  top: 8,
                  left: 8,
