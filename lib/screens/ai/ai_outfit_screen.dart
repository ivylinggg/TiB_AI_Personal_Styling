...
class _AIOutfitScreenState extends State<AIOutfitScreen> {
  ...
  Future<void> _loadWardrobe() async {
    ...
    try {
      ...
      if (mounted) setState(() { _wardrobe = items; _loading = false; });
    } catch (_) {
      if (mounted) setState(() => _loading = false);
    }
  }
  ...
  List<WardrobeItem> _buildLook(ColourAnalysisResult profile) {
    ...
    if (result.any((item) => item.category == 'Dresses')) {
      result.removeWhere((item) => item.category == 'Tops' || item.category == 'Bottoms');
    }
  ...
  Widget build(BuildContext context) {
    ...
    final look = profile == null ? const <WardrobeItem>[] : _buildLook(profile);
      ...
      body: _loading
          ...
          : ListView(
              ...
              children: [
                ...
                const Text('What are you dressing for?', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
                ...
                SizedBox(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (_, index) {
                      ...
                      return ChoiceChip(
                        selected: selected,
                        label: Row(mainAxisSize: MainAxisSize.min, children: [Icon(item.$2, size: 16), const SizedBox(width: 6), Text(item.$1)]),
                        onSelected: (_) => setState(() { _occasion = item.$1; _generated = false; }),
  ...
  Widget _itemCard(WardrobeItem item) => SizedBox(width: 150, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: ClipRRect(borderRadius: BorderRadius.circular(17), child: item.imageUrl.isEmpty ? Container(color: AppColors.surfaceMuted, width: double.infinity, child: const Center(child: Icon(Icons.checkroom_outlined, color: AppColors.primary))) : CachedNetworkImage(imageUrl: item.imageUrl, fit: BoxFit.cover, width: double.infinity))), const SizedBox(height: 8), Text(item.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 12)), const SizedBox(height: 2), Text('${item.category} · ${item.colour}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: AppColors.textMuted, fontSize: 10.5))]));

  Widget _message(String text) => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(20), border: Border.all(color: AppColors.border)), child: Text(text, style: const TextStyle(color: AppColors.textSecondary, height: 1.45, fontSize: 12.5)));
}
