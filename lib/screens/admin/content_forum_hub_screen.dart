...
class _ContentForumHubScreenState extends State<ContentForumHubScreen> {
              ...
              FilledButton(
                onPressed: saving ? null : () async {
                  ...
                  if (uid == null || title.isEmpty || body.isEmpty) {
                    ScaffoldMessenger.of(dialogContext).showSnackBar(const SnackBar(content: Text('Please enter a title and post body.')));
                    return;
                  }
                  ...
                  try {
                    ...
                    ScaffoldMessenger.of(dialogContext).showSnackBar(SnackBar(content: Text('Could not create post: $error')));
                  }
                },
  ...
  Future<void> _deletePost(QueryDocumentSnapshot<Map<String, dynamic>> doc) async {
    ...
    try {
      ...
      await batch.commit();
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Forum post removed.')));
    } catch (error) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Could not delete post: $error')));
    }
  }
    ...
    await Navigator.of(context).push<void>(MaterialPageRoute(builder: (_) => ForumPostDetailScreen(
      ...
      onAddComment: (text) async {
        ...
        try {
          final user = await FirebaseFirestore.instance.collection('users').doc(uid).get();
          final data = user.data() ?? <String, dynamic>{};
          final ref = doc.reference.collection('comments').doc();
          final batch = FirebaseFirestore.instance.batch();
          batch.set(ref, {'body': trimmed, 'authorId': uid, 'authorName': await _currentAuthorName(uid), 'authorRole': data['role'] as String? ?? 'admin', 'isOfficial': data['role'] == 'admin', 'createdAt': FieldValue.serverTimestamp()});
          batch.update(doc.reference, {'commentCount': FieldValue.increment(1), 'lastActivityAt': FieldValue.serverTimestamp()});
          await batch.commit();
          return true;
        } catch (_) {
          return false;
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          docs.sort((a, b) {
            final ao = _official(a.data());
            final bo = _official(b.data());
            if (ao != bo) return ao ? -1 : 1;
            final at = a.data()['lastActivityAt'] ?? a.data()['createdAt'];
            final bt = b.data()['lastActivityAt'] ?? b.data()['createdAt'];
            return _date(bt).compareTo(_date(at));
          });
          ...
          return Column(children: [
            Padding(padding: const EdgeInsets.fromLTRB(20, 16, 20, 8), child: Column(children: [
              TextField(controller: _searchController, onChanged: (v) => setState(() => _query = v.trim()), decoration: InputDecoration(hintText: 'Search forum', prefixIcon: const Icon(Icons.search), border: OutlineInputBorder(borderRadius: BorderRadius.circular(16)))),
            ...
            Expanded(child: visible.isEmpty ? const Center(child: Text('No forum posts found.')) : ListView.separated(
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                      if (official && contentId != null) IconButton(tooltip: 'Open Content', icon: const Icon(Icons.open_in_new_rounded), onPressed: () async {
                        try {
                      ...
                      PopupMenuButton<String>(onSelected: (value) { if (value == 'delete') _deletePost(doc); }, itemBuilder: (_) => const [PopupMenuItem(value: 'delete', child: Text('Delete'))]),
                    ]),
                  ),
                );
              },
