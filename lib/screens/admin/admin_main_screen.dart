...
class _AdminMainScreenState extends State<AdminMainScreen> {
  ...
  Future<void> _verifyAdministratorAccess() async {
    ...
    try {
      ...
      if (user == null) {
        if (!mounted) return;
        setState(() { _isCheckingAccess = false; _hasAdminAccess = false; _accessError = 'Your session has expired. Please sign in again.'; });
        return;
      }
      ...
      if (!mounted) return;
      setState(() { _isCheckingAccess = false; _hasAdminAccess = role == 'admin' && isActive; _accessError = role != 'admin' ? 'Administrator access is required for this dashboard.' : (!isActive ? 'This administrator account is inactive.' : null); });
    } catch (_) {
      if (!mounted) return;
      setState(() { _isCheckingAccess = false; _hasAdminAccess = false; _accessError = 'We could not verify administrator access. Please try again.'; });
    }
  }

  String get _modeLabel => switch (_mode) { AdminMode.administrator => 'Administrator', AdminMode.consultantPreview => 'Consultant Console', AdminMode.customerPreview => 'Customer Preview' };
  String get _modeDescription => switch (_mode) { AdminMode.administrator => 'Full administration access', AdminMode.consultantPreview => 'Respond to live customer consultations', AdminMode.customerPreview => 'Preview a real customer account without changing your admin session' };
  IconData get _modeIcon => switch (_mode) { AdminMode.administrator => Icons.admin_panel_settings_outlined, AdminMode.consultantPreview => Icons.support_agent_rounded, AdminMode.customerPreview => Icons.person_outline_rounded };

  void _setMode(AdminMode mode) {
    if (_mode == mode) return;
    if (mode != AdminMode.customerPreview) context.read<PreviewContext>().clear();
    setState(() { _mode = mode; _selectedIndex = 0; });
  }

  void _resetAdministratorMode() {
    context.read<PreviewContext>().clear();
    if (_mode == AdminMode.administrator && _selectedIndex == 0) return;
    setState(() { _mode = AdminMode.administrator; _selectedIndex = 0; });
  }
  ...
  Future<void> _selectCustomerPreview() async {
    try {
        ...
        builder: (sheetContext) => SafeArea(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                const ListTile(contentPadding: EdgeInsets.zero, title: Text('Choose a Customer'), subtitle: Text('Preview their real saved style data. Your admin session remains unchanged.')),
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Could not load customer list: ${error.message ?? error.code}')));
    }
  }
  ...
  List<Widget> get _pages => switch (_mode) {
    AdminMode.administrator => [AdminDashboardScreen(onNavigate: _navigateTo), const UserManagementScreen(), const AnalysisManagementScreen(), const ContentForumHubScreen(), const PremiumManagementScreen(), const AdminProfileScreen(), const ConsultationManagementScreen(), const StaffManagementScreen()],
    AdminMode.consultantPreview => [const ConsultationManagementScreen(), const AdminProfileScreen()],
    AdminMode.customerPreview => [const MainScreen(adminPreview: true), const AdminProfileScreen()],
  };

  List<NavigationDestination> get _destinations => switch (_mode) {
    AdminMode.administrator => const [NavigationDestination(icon: Icon(Icons.dashboard_outlined), selectedIcon: Icon(Icons.dashboard), label: 'Dashboard'), NavigationDestination(icon: Icon(Icons.people_outline), selectedIcon: Icon(Icons.people), label: 'Users'), NavigationDestination(icon: Icon(Icons.analytics_outlined), selectedIcon: Icon(Icons.analytics), label: 'Analysis'), NavigationDestination(icon: Icon(Icons.forum_outlined), selectedIcon: Icon(Icons.forum_rounded), label: 'Forum'), NavigationDestination(icon: Icon(Icons.workspace_premium_outlined), selectedIcon: Icon(Icons.workspace_premium), label: 'Premium'), NavigationDestination(icon: Icon(Icons.admin_panel_settings_outlined), selectedIcon: Icon(Icons.admin_panel_settings), label: 'Admin'), NavigationDestination(icon: Icon(Icons.support_agent_outlined), selectedIcon: Icon(Icons.support_agent), label: 'Consult'), NavigationDestination(icon: Icon(Icons.badge_outlined), selectedIcon: Icon(Icons.badge_rounded), label: 'Staff')],
    AdminMode.consultantPreview => const [NavigationDestination(icon: Icon(Icons.support_agent_outlined), selectedIcon: Icon(Icons.support_agent), label: 'Live Consultancy'), NavigationDestination(icon: Icon(Icons.admin_panel_settings_outlined), selectedIcon: Icon(Icons.admin_panel_settings), label: 'Admin')],
    AdminMode.customerPreview => const [NavigationDestination(icon: Icon(Icons.person_outline_rounded), selectedIcon: Icon(Icons.person_rounded), label: 'Customer'), NavigationDestination(icon: Icon(Icons.admin_panel_settings_outlined), selectedIcon: Icon(Icons.admin_panel_settings), label: 'Admin')],
  };

  void _navigateTo(int index) {
    if (index < 0) return;
    if ((_mode == AdminMode.consultantPreview || _mode == AdminMode.customerPreview) && index == 1) { _resetAdministratorMode(); return; }
    if (index >= _pages.length) return;
    setState(() => _selectedIndex = index);
  }

  Widget _buildAccessDenied() => Scaffold(appBar: AppBar(title: const Text('Administrator Access')), body: Center(child: Padding(padding: const EdgeInsets.all(28), child: Column(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.lock_outline_rounded, size: 64), const SizedBox(height: 18), const Text('Access Restricted', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800), textAlign: TextAlign.center), const SizedBox(height: 8), Text(_accessError ?? 'Administrator access is required.', textAlign: TextAlign.center), const SizedBox(height: 18), FilledButton.icon(onPressed: _isCheckingAccess ? null : _verifyAdministratorAccess, icon: const Icon(Icons.refresh_rounded), label: const Text('Check Again'))])));
