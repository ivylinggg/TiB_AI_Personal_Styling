        ...
        children: [
          StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              ...
              final label = onlineCount == 0
                  ? 'No consultants online'
                  : '$onlineCount consultant${onlineCount == 1 ? '' : 's'} online';
                ...
                child: Row(
                  children: [
                    Icon(
                      Icons.circle,
                      size: 9,
                      color: onlineCount > 0 ? Colors.green : AppColors.textMuted,
                    ),
            ...
            child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.hasError) {
                  return Center(
                    child: Text(
                      'Unable to load consultations: ${snapshot.error}',
                    ),
                  );
                }
                if (!snapshot.hasData) {
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final unread =
                        (data['unreadForConsultant'] as num?)?.toInt() ?? 0;
                    final assigned = data['assignedConsultantName'] as String?;
                    final lastMessage = data['lastMessage'] as String?;
                    final email = data['email'] as String?;
                    final userName =
                        data['userName'] as String? ?? 'TiB User';
                    final subtitle =
                        assigned ?? lastMessage ?? email ?? 'New consultation';
...
class _ConsultantChatScreenState extends State<ConsultantChatScreen> {
  ...
  Future<void> _send() async {
    ...
    final displayName = consultantName == null || consultantName.isEmpty
        ? 'TiB Consultant'
        : consultantName;
          ...
          return Column(
            children: [
              Expanded(
                child: docs.isEmpty
                    ? const Center(
                        child: Text('Customer has not sent a message yet.'),
                      )
                    : ListView.builder(
                        padding: const EdgeInsets.all(16),
                        itemCount: docs.length,
