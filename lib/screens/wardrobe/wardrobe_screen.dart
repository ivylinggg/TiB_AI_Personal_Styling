...
class _WardrobeScreenState extends State<WardrobeScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final medium = width >= 520 && width < 900;
          ...
          : StreamBuilder<List<WardrobeItem>>(
              ...
              builder: (context, snapshot) {
                ...
                final paletteCount = items
                    ...
                    .length;
                final contentMaxWidth = width >= 1100 ? 980.0 : double.infinity;
                final gridCount = compact
                    ? 2
                    : medium
                        ? 3
                        : width >= 1200
                            ? 4
                            : 3;
                final gridRatio = compact ? .68 : medium ? .72 : .76;
                        ...
                        slivers: [
                          ...
                          if (items.isNotEmpty)
                            SliverToBoxAdapter(
                              child: _reveal(
                                _summaryReveal,
                                _buildSummaryRow(items, horizontal),
                              ),
                            ),
  ...
  Widget _buildSearchAndFilterBar(bool hasActiveFilters, bool compact) {
    final search = Expanded(child: _buildSearchBar());
    final filter = Semantics(
      button: true,
      label: _filtersExpanded
          ? 'Collapse wardrobe filters'
          : 'Open wardrobe filters',
      child: Material(
  ...
  Widget _buildFilterPanel(double horizontal) {
    return Padding(
      padding: EdgeInsets.fromLTRB(horizontal, 10, horizontal, 4),
      child: Container(
  ...
  Widget _buildActiveFilters(String query, int filteredCount, int totalCount) {
    final chips = <Widget>[];
    if (_category != 'All') {
      chips.add(_buildActiveFilterChip(label: _category, onRemove: () => setState(() => _category = 'All')));
    }
    if (_colour != 'All') {
      chips.add(_buildActiveFilterChip(label: _colour, onRemove: () => setState(() => _colour = 'All')));
    }
    if (_showFavouritesOnly) {
      chips.add(_buildActiveFilterChip(label: 'Favourites', icon: Icons.favorite_rounded, onRemove: () => setState(() => _showFavouritesOnly = false)));
    }
    if (query.isNotEmpty) {
      chips.add(_buildActiveFilterChip(label: 'Search: ${_searchQuery.trim()}', onRemove: _searchController.clear));
    }
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
  ...
  Widget _buildPremiumInsightCard(List<WardrobeItem> items, int paletteCount, List<String> wantedColours, double horizontal, bool compact) {
    ...
    return Padding(
      padding: EdgeInsets.fromLTRB(horizontal, 2, horizontal, 12),
      child: Container(
          ...
          children: [
            ...
            LayoutBuilder(
              builder: (context, constraints) {
                final narrow = compact || constraints.maxWidth < 430;
                final cardWidth = narrow ? (constraints.maxWidth - 8) / 2 : (constraints.maxWidth - 16) / 3;
                return Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: [
                    SizedBox(width: cardWidth, child: _insightStat('Pieces', '${items.length}')),
                    SizedBox(width: cardWidth, child: _insightStat('Favourites', '$favouriteCount')),
                    SizedBox(width: narrow ? constraints.maxWidth : cardWidth, child: _insightStat('Top colour', mostCommon(colourCounts))),
                  ],
                );
              },
            ),
  ...
  String _headlineFor(int count) => count == 0
      ? 'Your closet is empty'
      : count == 1
          ? '1 piece'
          : '$count pieces';

  Widget _buildHeader(String uid, int totalCount, double horizontal, bool compact) {
    final content = Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('MY WARDROBE', style: TextStyle(color: _brown, fontSize: 12, fontWeight: FontWeight.w800, letterSpacing: 1.2)),
        const SizedBox(height: 6),
        Text(_headlineFor(totalCount), style: TextStyle(color: _text, fontSize: compact ? 25 : 26, fontWeight: FontWeight.w800, letterSpacing: -.4)),
        const SizedBox(height: 4),
        const Text('Pieces you love, ready to style.', style: TextStyle(color: _muted, fontSize: 13, height: 1.4)),
      ],
    );
    return Padding(
      padding: EdgeInsets.fromLTRB(horizontal, compact ? 8 : 10, horizontal, 2),
      child: compact
          ? Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [content, const SizedBox(height: 10), _headerAddButton(uid)],
            )
          : Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [Expanded(child: content), const SizedBox(width: 10), _headerAddButton(uid)],
            ),
    );
  }

  Widget _headerAddButton(String uid) {
    return Semantics(
      button: true,
      label: 'Add clothing to your wardrobe',
  ...
  Widget _buildSummaryRow(List<WardrobeItem> items, double horizontal) {
    ...
    final chips = counts.entries.map<Widget>((entry) => Container(
  ...
  Widget _buildEmptyState({required bool hasAnyItems, required String uid}) {
    ...
    return EmptyState(
      ...
      description: 'Try a different category, search term or turn off the favourites filter.',
                    ...
                    children: [
                      Positioned.fill(
                        child: item.imageUrl.isEmpty
                            ? Container(
                                color: _soft,
                                child: Icon(_categoryIcon(item.category), size: 38, color: _brown),
                              )
                            : CachedNetworkImage(
                          ...
                          child: Container(
                            ...
                            child: const Row(
                              ...
                              children: [
                                ...
                                SizedBox(width: 3),
                                Text('Palette match', style: TextStyle(color: AppColors.success, fontSize: 9.5, fontWeight: FontWeight.w800)),
                              ],
                            ),
                          ),
                        ),
        ...
        SnackBar(
          ...
          action: SnackBarAction(
            ...
            onPressed: () {
              ...
              if (currentUid != null) _showAddItem(currentUid);
            },
          ),
        ),
      );
      ...
      builder: (sheetContext) => StatefulBuilder(
        builder: (context, setSheetState) {
          Future<void> save() async {
            ...
            try {
              final rejection = await WardrobeImageValidationService.validate(
                image,
                category: category,
              );
              if (rejection != null) {
                if (sheetContext.mounted) {
                  ...
                  ScaffoldMessenger.of(sheetContext).showSnackBar(
                    SnackBar(content: Text(rejection), behavior: SnackBarBehavior.floating),
                  );
                }
                return;
              }
              ...
              if (sheetContext.mounted) {
                ScaffoldMessenger.of(sheetContext).showSnackBar(
                  const SnackBar(content: Text('Could not save this piece. Please try again.')),
                );
              }
            } finally {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      decoration: _fieldDecoration('Name', hint: 'e.g. Cream knit cardigan'),
                    ),
                    const SizedBox(height: 12),
  ...
  Future<void> _toggleFavourite(WardrobeItem item, String uid) async {
    try {
      await FirestoreService.updateWardrobeItem(
        uid,
        item.id,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not update favourite. Please try again.')),
        );
      }
    }
  }
  ...
  Widget _detailRow(String label, String value) => Padding(
        ...
        child: Row(
          ...
          children: [
            SizedBox(
              width: 90,
              child: Text(label, style: const TextStyle(color: _muted, fontSize: 12, fontWeight: FontWeight.w600)),
            ),
            Expanded(
              child: Text(value, style: const TextStyle(color: _text, fontWeight: FontWeight.w600)),
            ),
          ],
        ),
      );
  ...
  Future<void> _confirmDelete(WardrobeItem item, String uid) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not remove this piece. Please try again.')),
        );
      }
    }
  }
                ...
                if (matchesPalette) ...[
                  ...
                  const Row(
                    children: [
                      ...
                      SizedBox(width: 5),
                      Text('Matches your colour palette', style: TextStyle(color: AppColors.success, fontSize: 12, fontWeight: FontWeight.w700)),
                    ],
                  ),
                ],
                const SizedBox(height: 16),
        ...
        builder: (context, setSheetState) {
          Future<void> save() async {
            ...
            try {
              ...
              if (sheetContext.mounted) {
                ScaffoldMessenger.of(sheetContext).showSnackBar(
                  const SnackBar(content: Text('Could not update this piece. Please try again.')),
                );
              }
            } finally {
              if (sheetContext.mounted) setSheetState(() => saving = false);
                    ...
                    ClipRRect(
                      ...
                      child: item.imageUrl.isEmpty
                          ? Container(
                              ...
                              width: double.infinity,
                              color: _soft,
                              child: Icon(_categoryIcon(item.category), size: 48, color: _brown),
                            )
                          : CachedNetworkImage(
                              imageUrl: item.imageUrl,
                              height: 190,
