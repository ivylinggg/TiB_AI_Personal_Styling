...
enum _WardrobeView { all, favourites }
...
class _WardrobeScreenState extends State<WardrobeScreen> with SingleTickerProviderStateMixin {
      ...
      body: uid == null
          ...
          : StreamBuilder<List<WardrobeItem>>(
              ...
              builder: (context, snapshot) {
                ...
                if (_sort == 'Category') filtered.sort((a, b) => a.category.toLowerCase().compareTo(b.category.toLowerCase()));
                if (_sort == 'Favourites first') filtered.sort((a, b) => (b.isFavourite ? 1 : 0).compareTo(a.isFavourite ? 1 : 0));
  ...
  Widget _buildSearchAndFilterBar(bool hasActiveFilters) {
    return Row(children: [Expanded(child: _buildSearchBar()), const SizedBox(width: 10), Material(color: _filtersExpanded || hasActiveFilters ? AppColors.primarySoft : AppColors.surface, borderRadius: BorderRadius.circular(AppRadius.full), child: InkWell(borderRadius: BorderRadius.circular(AppRadius.full), onTap: () => setState(() => _filtersExpanded = !_filtersExpanded), child: Padding(padding: const EdgeInsets.symmetric(horizontal: 15, vertical: 13), child: Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.tune_rounded, size: 18, color: _brown), const SizedBox(width: 6), const Text('Filter', style: TextStyle(color: _brown, fontSize: 12, fontWeight: FontWeight.w800)), const SizedBox(width: 3), Icon(_filtersExpanded ? Icons.keyboard_arrow_up_rounded : Icons.keyboard_arrow_down_rounded, size: 18, color: _brown)]))))]);
  }
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 14),
          Row(children: [Expanded(child: OutlinedButton.icon(onPressed: _clearFilters, icon: const Icon(Icons.clear_rounded, size: 17), label: const Text('Clear'))), const SizedBox(width: 9), Expanded(child: FilledButton.icon(onPressed: () => setState(() => _filtersExpanded = false), icon: const Icon(Icons.check_rounded, size: 17), label: const Text('Done')))]),
        ]),
      ),
    );
  }
  ...
  Widget _buildActiveFilterChip({required String label, required VoidCallback onRemove, IconData? icon}) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 6), decoration: BoxDecoration(color: AppColors.primarySoft, borderRadius: BorderRadius.circular(AppRadius.full)), child: Row(mainAxisSize: MainAxisSize.min, children: [if (icon != null) ...[Icon(icon, size: 12, color: _brown), const SizedBox(width: 4)], Flexible(child: Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: _brown))), const SizedBox(width: 4), const Icon(Icons.close_rounded, size: 13, color: _brown)]));
  }

  void _clearFilters() {
    _searchController.clear();
    setState(() { _category = 'All'; _colour = 'All'; _sort = 'Recently added'; _showFavouritesOnly = false; _filtersExpanded = false; });
  }
  ...
  Widget _buildPremiumInsightCard(List<WardrobeItem> items) {
    ...
    for (final item in items) { categoryCounts[item.category] = (categoryCounts[item.category] ?? 0) + 1; colourCounts[item.colour] = (colourCounts[item.colour] ?? 0) + 1; }
    String mostCommon(Map<String, int> values) { if (values.isEmpty) return '—'; var bestKey = values.keys.first; var bestValue = values[bestKey] ?? 0; for (final entry in values.entries) { if (entry.value > bestValue) { bestKey = entry.key; bestValue = entry.value; } } return bestKey; }
  ...
  bool _matchesPalette(WardrobeItem item, List<String> wantedColours) { if (wantedColours.isEmpty) return false; final itemColour = item.colour.toLowerCase(); return wantedColours.any((colour) => colour.contains(itemColour) || itemColour.contains(colour)); }
  ...
  Future<void> _showItemForm(String uid, File image) async {
    ...
    await showModalBottomSheet<void>(context: context, isScrollControlled: true, showDragHandle: true, backgroundColor: _cream, builder: (sheetContext) => StatefulBuilder(builder: (context, setSheetState) {
      Future<void> save() async {
        ...
        } finally { if (sheetContext.mounted) setSheetState(() => saving = false); }
      }
  ...
  Future<void> _toggleFavourite(WardrobeItem item, String uid) async { try { await FirestoreService.updateWardrobeItem(uid, item.id, {'isFavourite': !item.isFavourite}); } catch (_) { if (!mounted) return; ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not update favourite. Please try again.'))); } }
  ...
  Future<void> _confirmDelete(WardrobeItem item, String uid) async {
    ...
    try { await FirestoreService.deleteWardrobeItem(uid, item.id); if (mounted) Navigator.pop(context); } catch (_) { if (!mounted) return; ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not remove this piece. Please try again.'))); }
  }
  ...
  void _showItemDetails(WardrobeItem item, String uid) {
    ...
    final wantedColours = (analysisResult?.colours ?? const <String>[]).map((colour) => colour.toLowerCase()).toList();
    final matchesPalette = _matchesPalette(item, wantedColours);
    showModalBottomSheet<void>(context: context, showDragHandle: true, isScrollControlled: true, backgroundColor: _cream, builder: (context) => SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(20, 5, 20, 25), child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [ClipRRect(borderRadius: BorderRadius.circular(AppRadius.lg), child: AspectRatio(aspectRatio: 4 / 3, child: item.imageUrl.isEmpty ? Container(color: _soft, child: Icon(_categoryIcon(item.category), size: 46, color: _brown)) : CachedNetworkImage(imageUrl: item.imageUrl, fit: BoxFit.cover))), const SizedBox(height: 16), Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: Text(item.name, style: const TextStyle(color: _text, fontSize: 21, fontWeight: FontWeight.w700))), Material(color: _soft, shape: const CircleBorder(), child: IconButton(tooltip: item.isFavourite ? 'Remove from favourites' : 'Save as favourite', onPressed: () => _toggleFavourite(item, uid), icon: Icon(item.isFavourite ? Icons.favorite : Icons.favorite_border, color: item.isFavourite ? AppColors.premiumAccent : _brown)))]), if (matchesPalette) ...[const SizedBox(height: 4), const Row(children: [Icon(Icons.check_circle_rounded, size: 14, color: AppColors.success), SizedBox(width: 5), Text('Matches your colour palette', style: TextStyle(color: AppColors.success, fontSize: 12, fontWeight: FontWeight.w700))])], const SizedBox(height: 16), const Text('DETAILS', style: TextStyle(color: _muted, fontSize: 10.5, fontWeight: FontWeight.w800, letterSpacing: .6)), const SizedBox(height: 10), _detailRow('Colour', item.colour), _detailRow('Category', item.category), _detailRow('Style', item.style), if (item.season.isNotEmpty && item.season != 'All seasons') _detailRow('Season', item.season), if (item.notes.isNotEmpty) ...[const SizedBox(height: 4), const Text('NOTES', style: TextStyle(color: _muted, fontSize: 10.5, fontWeight: FontWeight.w800, letterSpacing: .6)), const SizedBox(height: 6), Text(item.notes, style: const TextStyle(color: _text, height: 1.4))], const SizedBox(height: 20), Row(children: [Expanded(child: OutlinedButton.icon(onPressed: () { Navigator.pop(context); _showEditItem(item, uid); }, icon: const Icon(Icons.edit_outlined), label: const Text('Edit'))), const SizedBox(width: 10), Expanded(child: FilledButton.icon(onPressed: () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (_) => AIStylistScreen(selectedItem: item))); }, icon: const Icon(Icons.auto_awesome_rounded), label: const Text('Style this'), style: FilledButton.styleFrom(backgroundColor: _brown)))]), const SizedBox(height: 10), SizedBox(width: double.infinity, child: TextButton.icon(onPressed: () => _confirmDelete(item, uid), icon: const Icon(Icons.delete_outline), label: const Text('Delete'), style: TextButton.styleFrom(foregroundColor: AppColors.error))])))));
  }
