...
class _WardrobeScreenState extends State<WardrobeScreen> with SingleTickerProviderStateMixin {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          Semantics(
            button: true,
            label: _showFavouritesOnly ? 'Show all wardrobe items' : 'Show favourite wardrobe items',
            toggled: _showFavouritesOnly,
            child: IconButton(
              tooltip: _showFavouritesOnly ? 'Show all items' : 'Favourites',
          ...
          Semantics(
            button: true,
            label: _isPremium ? 'Open Smart Wardrobe' : 'Smart Wardrobe premium feature',
            child: IconButton(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Could not load your wardrobe.', style: TextStyle(color: _text, fontWeight: FontWeight.w700, fontSize: 16)),
            const SizedBox(height: 6),
            const Text('Please check your connection and try again.', textAlign: TextAlign.center, style: TextStyle(color: _muted, height: 1.4)),
            const SizedBox(height: 16),
            OutlinedButton.icon(onPressed: _isRefreshing ? null : _retryWardrobeLoad, icon: _isRefreshing ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.refresh_rounded), label: Text(_isRefreshing ? 'Retrying...' : 'Try again')),
          ],
        ),
      ),
    );
  ...
  Future<void> _retryWardrobeLoad() async {
    if (_isRefreshing) return;
    setState(() => _isRefreshing = true);
    await Future<void>.delayed(const Duration(milliseconds: 300));
    if (mounted) setState(() => _isRefreshing = false);
  ...
  Widget _buildSearchAndFilterBar(bool hasActiveFilters) {
    return Row(
      children: [
        ...
        Semantics(
          button: true,
          label: _filtersExpanded ? 'Collapse wardrobe filters' : 'Open wardrobe filters',
          child: Material(
  ...
  Widget _buildSummaryRow(List<WardrobeItem> items) {
    ...
    final chips = counts.entries.map<Widget>((entry) {
      return Semantics(label: '${entry.value} ${entry.key}', child: Container(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 15),
                    TextField(controller: nameController, textInputAction: TextInputAction.next, decoration: _fieldDecoration('Name', hint: 'e.g. Cream knit cardigan')),
                    const SizedBox(height: 12),
                    ...
                    const SizedBox(height: 4),
                    TextField(controller: notesController, maxLines: 2, textInputAction: TextInputAction.done, decoration: _fieldDecoration('Notes (optional)')),
                    const SizedBox(height: 16),
                    Semantics(button: true, label: saving ? 'Saving wardrobe item' : 'Save clothing to my wardrobe', child: SizedBox(
        ...
        child: Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Semantics(image: true, label: '${item.name} wardrobe image', child: ClipRRect(
                  borderRadius: BorderRadius.circular(AppRadius.lg),
                  child: AspectRatio(
                    aspectRatio: 4 / 3,
                    child: item.imageUrl.isEmpty
                ...
                if (matchesPalette) ...[
                  const SizedBox(height: 4),
                  const Semantics(label: 'Matches your colour palette', child: Row(children: [Icon(Icons.check_circle_rounded, size: 14, color: AppColors.success), SizedBox(width: 5), Text('Matches your colour palette', style: TextStyle(color: AppColors.success, fontSize: 12, fontWeight: FontWeight.w700))])),
                ],
                const SizedBox(height: 16),
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 15),
                    Semantics(image: true, label: '${item.name} wardrobe image', child: ClipRRect(borderRadius: BorderRadius.circular(AppRadius.lg), child: item.imageUrl.isEmpty ? Container(height: 190, width: double.infinity, color: _soft, child: Icon(_categoryIcon(item.category), size: 46, color: _brown)) : CachedNetworkImage(imageUrl: item.imageUrl, height: 190, width: double.infinity, fit: BoxFit.cover))),
                    const SizedBox(height: 15),
                    TextField(controller: nameController, textInputAction: TextInputAction.next, decoration: _fieldDecoration('Name')),
                    const SizedBox(height: 12),
                    _dropdown('Category', category, _categoryOptions, (value) => setSheetState(() => category = value)),
                    _dropdown('Colour', colour, _colourOptions, (value) => setSheetState(() => colour = value)),
                    _dropdown('Style', style, _styleOptions, (value) => setSheetState(() => style = value)),
                    ...
                    TextField(controller: notesController, maxLines: 2, textInputAction: TextInputAction.done, decoration: _fieldDecoration('Notes (optional)')),
                    const SizedBox(height: 16),
                    Semantics(button: true, label: saving ? 'Saving wardrobe item changes' : 'Save changes to wardrobe item', child: SizedBox(
                      width: double.infinity,
                      child: FilledButton.icon(
                        onPressed: saving ? null : save,
