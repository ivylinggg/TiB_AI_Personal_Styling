...
class _WardrobeScreenState extends State<WardrobeScreen>
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed:
                uid == null ? null : () => _showSmartWardrobe(uid),
            icon: Icon(
              _isPremium
  ...
  Widget _premiumInsightListTile(
    IconData icon,
    String title,
    String subtitle,
  ) {
  ...
  Widget _buildPremiumInsightCard(
    ...
    final paletteText = wantedColours.isEmpty
        ? 'Run Colour Analysis to unlock palette matching.'
        : '$paletteCount of ${items.length} pieces match your personal palette.';
    return Padding(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
              ...
              builder: (context, constraints) {
                ...
                return Wrap(
                  ...
                  children: [
                    SizedBox(
                      width: cardWidth,
                      child: _insightStat('Pieces', '${items.length}'),
                    ),
                    SizedBox(
                      width: cardWidth,
                      child: _insightStat('Favourites', '$favouriteCount'),
                    ),
                    SizedBox(
                      width: narrow ? constraints.maxWidth : cardWidth,
                      child: _insightStat(
                        'Top colour',
                        mostCommon(colourCounts),
                      ),
                    ),
                  ],
                );
            ...
            const SizedBox(height: 9),
            Text(
              'Most used category: ${mostCommon(categoryCounts)}',
              style: const TextStyle(
  ...
  Widget _buildEmptyState({
    ...
    if (!hasAnyItems) {
      return EmptyState(
        ...
        description:
            'Add a few pieces you already love and your AI stylist can start creating looks from them.',
    ...
    return EmptyState(
      ...
      description:
          'Try a different category, search term or turn off the favourites filter.',
  ...
  bool _matchesPalette(WardrobeItem item, List<String> wantedColours) {
    ...
    return wantedColours.any((colour) {
      ...
      return normalized == itemColour ||
          normalized.contains(itemColour) ||
          itemColour.contains(normalized);
    });
  }
  ...
  Widget _buildItemCard(
    ...
    return Semantics(
      container: true,
      label:
          '${item.name}, ${item.category}, ${item.colour}${item.isFavourite ? ', favourite' : ''}${matchesPalette ? ', palette match' : ''}',
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              const Text(
                'Only clothing and fashion accessories are accepted. Clear photos of a single wearable item work best.',
                      ...
                      child: FilledButton.icon(
                        onPressed: () {
                          ...
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (_) =>
                                  AIStylistScreen(selectedItem: item),
                            ),
                          );
                        },
            ...
            try {
              ...
              if (sheetContext.mounted) {
                ScaffoldMessenger.of(sheetContext).showSnackBar(
                  const SnackBar(
                    content: Text(
                      'Could not update this piece. Please try again.',
                    ),
                  ),
                );
              }
