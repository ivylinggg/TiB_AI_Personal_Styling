...
class _WardrobeScreenState extends State<WardrobeScreen>
  ...
  static const _colourOptions = [
    'Black', 'White', 'Beige', 'Brown', 'Pink', 'Red', 'Orange', 'Yellow', 'Green', 'Blue', 'Purple', 'Neutral',
  ];
  static const _styleOptions = [
    'Everyday', 'Minimal', 'Elegant', 'Casual', 'Smart Casual', 'Feminine', 'Trendy',
  ];
  ...
  Widget _reveal(Animation<double> animation, Widget child) => AnimatedBuilder(
        ...
        builder: (context, animatedChild) {
          ...
          return Opacity(
            opacity: value,
            child: Transform.translate(offset: Offset(0, (1 - value) * 14), child: animatedChild),
          );
        },
        child: child,
      );
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            icon: Icon(_showFavouritesOnly ? Icons.favorite : Icons.favorite_border,
              ...
              builder: (context, snapshot) {
                ...
                final filtered = items.where((item) {
                  ...
                  final searchMatches = query.isEmpty || item.name.toLowerCase().contains(query) ||
                      item.category.toLowerCase().contains(query) || item.colour.toLowerCase().contains(query) || item.style.toLowerCase().contains(query);
                ...
                return RefreshIndicator(
                  ...
                  child: CustomScrollView(
                    ...
                    slivers: [
                      ...
                      SliverToBoxAdapter(child: _reveal(_browseReveal, Padding(
                        padding: const EdgeInsets.fromLTRB(20, 14, 20, 0),
                        child: _buildSearchAndFilterBar(hasActiveFilters),
                      ))),
                      if (hasActiveFilters) SliverToBoxAdapter(child: Padding(
                        ...
                        SliverPadding(
                          ...
                          sliver: SliverGrid(
                            delegate: SliverChildBuilderDelegate(
                              (context, index) => _buildItemCard(filtered[index], uid, index, wantedColours),
                              childCount: filtered.length,
                            ),
                            gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                              crossAxisCount: 2, crossAxisSpacing: 12, mainAxisSpacing: 14, childAspectRatio: .70,
                            ),
                          ),
                        ),
                    ],
          ...
          child: InkWell(
            ...
            child: Padding(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                const Text('Sort & Filter', style: TextStyle(color: _brown, fontSize: 12, fontWeight: FontWeight.w800)),
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Wrap(spacing: 7, runSpacing: 7, children: ['Recently added', 'Name A–Z', 'Category', 'Favourites first'].map((value) => StyleChip(
            label: value, icon: Icons.sort_rounded, selected: _sort == value, onTap: () => setState(() => _sort = value),
          )).toList()),
          const SizedBox(height: 14),
          const Text('CATEGORY', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: _muted, letterSpacing: .8)),
          const SizedBox(height: 8),
          Wrap(spacing: 7, runSpacing: 7, children: ['All', ..._categoryOptions].map((value) => StyleChip(
            label: value, icon: value == 'All' ? Icons.grid_view_rounded : _categoryIcon(value), selected: _category == value,
            onTap: () => setState(() => _category = value),
          )).toList()),
          const SizedBox(height: 14),
          const Text('COLOUR', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: _muted, letterSpacing: .8)),
          const SizedBox(height: 8),
          Wrap(spacing: 7, runSpacing: 7, children: ['All', ..._colourOptions].map((value) => StyleChip(
            label: value, icon: Icons.palette_outlined, selected: _colour == value, onTap: () => setState(() => _colour = value),
          )).toList()),
          const SizedBox(height: 14),
          Row(children: [
            ...
            Expanded(child: FilledButton.icon(onPressed: () => setState(() => _filtersExpanded = false), icon: const Icon(Icons.check_rounded, size: 17), label: const Text('Apply'), style: FilledButton.styleFrom(backgroundColor: _brown))),
          ]),
        ]),
      ),
    );
  ...
  Widget _buildActiveFilters(String query, int filteredCount, int totalCount) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        const Spacer(),
        TextButton(onPressed: _clearFilters, style: TextButton.styleFrom(padding: EdgeInsets.zero, minimumSize: const Size(0, 28), tapTargetSize: MaterialTapTargetSize.shrinkWrap), child: const Text('Clear all', style: TextStyle(fontSize: 11))),
      ]),
      Wrap(spacing: 6, runSpacing: 6, children: [
        if (_category != 'All') _buildActiveFilterChip(label: _category, onRemove: () => setState(() => _category = 'All')),
        if (_colour != 'All') _buildActiveFilterChip(label: _colour, onRemove: () => setState(() => _colour = 'All')),
        if (_showFavouritesOnly) _buildActiveFilterChip(label: 'Favourites', icon: Icons.favorite_rounded, onRemove: () => setState(() => _showFavouritesOnly = false)),
        if (query.isNotEmpty) _buildActiveFilterChip(label: 'Search: ${_searchQuery.trim()}', onRemove: _searchController.clear),
      ]),
  ...
  void _showSmartWardrobe(String uid) {
    ...
    showModalBottomSheet<void>(
      context: context, showDragHandle: true, backgroundColor: _cream,
      builder: (sheetContext) => SafeArea(child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          SizedBox(width: double.infinity, child: FilledButton.icon(
            onPressed: () { Navigator.pop(sheetContext); Navigator.push(context, MaterialPageRoute(builder: (_) => const AIStylistScreen())); },
            icon: const Icon(Icons.auto_awesome), label: const Text('Open AI Stylist'), style: FilledButton.styleFrom(backgroundColor: _brown, minimumSize: const Size.fromHeight(52)),
          )),
        ]),
      )),
    );
    ...
    return Padding(padding: const EdgeInsets.only(bottom: 10), child: Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(subtitle, style: const TextStyle(color: _muted, fontSize: 12, height: 1.35)),
        ])),
      ]),
    ));
  }
  ...
  Widget _buildPremiumInsightCard(List<WardrobeItem> items) {
    ...
    return Padding(padding: const EdgeInsets.fromLTRB(18, 2, 18, 12), child: Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          const SizedBox(width: 8), Expanded(child: _insightStat('Favourites', '$favouriteCount')),
          const SizedBox(width: 8), Expanded(child: _insightStat('Top colour', mostCommon(colourCounts))),
        ]),
  ...
  String _headlineFor(int count) {
    if (count == 0) return 'Your closet is empty';
  ...
  Widget _buildHeader(String uid, int totalCount) => Padding(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('MY WARDROBE', style: TextStyle(color: _brown, fontSize: 12, fontWeight: FontWeight.w800, letterSpacing: 1.2)),
        const SizedBox(height: 6),
        Text(_headlineFor(totalCount), style: const TextStyle(color: _text, fontSize: 26, fontWeight: FontWeight.w800, letterSpacing: -0.4)),
        const SizedBox(height: 4),
        const Text('Pieces you love, ready to style.', style: TextStyle(color: _muted, fontSize: 13, height: 1.4)),
      ])),
      const SizedBox(width: 10),
      TextButton.icon(onPressed: () => _showAddItem(uid), icon: const Icon(Icons.add_rounded, size: 18), label: const Text('Add'), style: TextButton.styleFrom(
        foregroundColor: _brown, backgroundColor: _soft, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppRadius.full)), padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
      )),
    ]),
  );
  ...
  Widget _buildSummaryRow(List<WardrobeItem> items) {
    ...
    return Padding(padding: const EdgeInsets.fromLTRB(20, 14, 20, 0), child: Wrap(spacing: 8, runSpacing: 8, children: counts.entries.map((entry) => Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(width: 5), Text(entry.key, style: const TextStyle(color: _muted, fontSize: 12, fontWeight: FontWeight.w600)),
      ]),
    )).toList()));
  }
  ...
  Widget _buildSearchBar() => Container(
    ...
    child: TextField(
      ...
      decoration: InputDecoration(
        hintText: 'Search your wardrobe...', hintStyle: const TextStyle(color: _muted, fontSize: 14), prefixIcon: const Icon(Icons.search_rounded, color: _muted),
  ...
  Widget _buildActiveFilterChip({required String label, required VoidCallback onRemove, IconData? icon}) => Container(
    ...
    child: InkWell(borderRadius: BorderRadius.circular(99), onTap: onRemove, child: Padding(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(width: 4), const Icon(Icons.close_rounded, size: 13, color: AppColors.primary),
      ]),
    )),
  );
  ...
  Widget _buildEmptyState({required bool hasAnyItems, required String uid}) {
    if (!hasAnyItems) return EmptyState(
      icon: Icons.checkroom_outlined, title: 'Your Closet Starts Here',
      description: 'Add a few pieces you already love and your AI stylist can start creating looks from them.',
      ctaLabel: 'Add your first piece', onCta: () => _showAddItem(uid),
    );
  ...
  Widget _buildItemCard(WardrobeItem item, String uid, int index, List<String> wantedColours) {
    ...
    return TweenAnimationBuilder<double>(
      ...
      child: Material(
        ...
        child: InkWell(onTap: () => _showItemDetails(item, uid), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Expanded(child: Stack(children: [
            Positioned.fill(child: item.imageUrl.isEmpty ? Container(color: _soft, child: Icon(_categoryIcon(item.category), size: 38, color: _brown)) : CachedNetworkImage(
              imageUrl: item.imageUrl, fit: BoxFit.cover,
              placeholder: (context, url) => Container(color: _soft, child: const Center(child: SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2)))),
              errorWidget: (context, url, error) => Container(color: _soft, child: Icon(_categoryIcon(item.category), color: _brown)),
            )),
            Positioned(top: 8, right: 8, child: Material(color: AppColors.background.withValues(alpha: .92), shape: const CircleBorder(), child: IconButton(
              padding: EdgeInsets.zero, constraints: const BoxConstraints.tightFor(width: 36, height: 36), onPressed: () => _toggleFavourite(item, uid),
            ...
            if (matchesPalette) Positioned(left: 8, bottom: 8, child: Container(
          ...
          Padding(padding: const EdgeInsets.fromLTRB(12, 9, 12, 11), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 4),
            Row(children: [
              Container(width: 9, height: 9, decoration: BoxDecoration(color: ColourNameMapper.colourFor(item.colour), shape: BoxShape.circle)),
              const SizedBox(width: 5), Expanded(child: Text('${item.category} · ${item.colour}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _muted, fontSize: 11.5))),
            ]),
          ]),
        ])),
      ),
  ...
  Widget _photoSourceTile({required IconData icon, required String label, required VoidCallback onTap}) => Material(
    color: AppColors.surfaceMuted, borderRadius: BorderRadius.circular(AppRadius.lg),
    child: InkWell(borderRadius: BorderRadius.circular(AppRadius.lg), onTap: onTap, child: Container(
      height: 110, decoration: BoxDecoration(borderRadius: BorderRadius.circular(AppRadius.lg), border: Border.all(color: AppColors.border)),
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, size: 30, color: _brown), const SizedBox(height: 8), Text(label, style: const TextStyle(color: _text, fontWeight: FontWeight.w700))],
    )),
  );

  Future<void> _showAddItem(String uid) async {
    final image = await showModalBottomSheet<File?>(context: context, showDragHandle: true, backgroundColor: _cream, builder: (context) => SafeArea(child: Padding(
      padding: const EdgeInsets.fromLTRB(20, 8, 20, 25),
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    await showModalBottomSheet<void>(context: context, isScrollControlled: true, showDragHandle: true, backgroundColor: _cream, builder: (sheetContext) => StatefulBuilder(builder: (context, setSheetState) {
      ...
      return SafeArea(child: Padding(
        ...
        child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 15), TextField(controller: nameController, decoration: _fieldDecoration('Name', hint: 'e.g. Cream knit cardigan')),
          const SizedBox(height: 12), _dropdown('Category', category, _categoryOptions, (v) => setSheetState(() => category = v)),
          _dropdown('Colour', colour, _colourOptions, (v) => setSheetState(() => colour = v)), _dropdown('Style', style, _styleOptions, (v) => setSheetState(() => style = v)), _dropdown('Season', season, _seasonOptions, (v) => setSheetState(() => season = v)),
          const SizedBox(height: 4), TextField(controller: notesController, maxLines: 2, decoration: _fieldDecoration('Notes (optional)')),
          const SizedBox(height: 16), SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: saving ? null : save, icon: saving ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: AppColors.background)) : const Icon(Icons.check_rounded), label: Text(saving ? 'Saving...' : 'Save to My Wardrobe'), style: FilledButton.styleFrom(backgroundColor: _brown, minimumSize: const Size.fromHeight(52)))),
        ])),
      ));
    }));
  }
  ...
  Widget _dropdown(String label, String value, List<String> values, ValueChanged<String> onChanged) => Padding(
    ...
    child: DropdownButtonFormField<String>(initialValue: value, decoration: _fieldDecoration(label), items: values.map((v) => DropdownMenuItem(value: v, child: Text(v))).toList(), onChanged: (v) { if (v != null) onChanged(v); }),
  );
  ...
  Future<void> _confirmDelete(WardrobeItem item, String uid) async {
    final confirmed = await showDialog<bool>(context: context, builder: (dialogContext) => AlertDialog(
      title: const Text('Remove this piece?'), content: Text('“${item.name}” will be removed from your wardrobe.'),
      actions: [TextButton(onPressed: () => Navigator.pop(dialogContext, false), child: const Text('Keep it')), FilledButton(onPressed: () => Navigator.pop(dialogContext, true), child: const Text('Remove'))],
    ));
  ...
  void _showItemDetails(WardrobeItem item, String uid) {
    ...
    final wantedColours = (analysisResult?.colours ?? const <String>[]).map((c) => c.toLowerCase()).toList();
    final matchesPalette = _matchesPalette(item, wantedColours);
    showModalBottomSheet<void>(context: context, showDragHandle: true, isScrollControlled: true, backgroundColor: _cream, builder: (context) => SafeArea(child: Padding(
      padding: const EdgeInsets.fromLTRB(20, 5, 20, 25),
      child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ClipRRect(borderRadius: BorderRadius.circular(AppRadius.lg), child: AspectRatio(aspectRatio: 4 / 3, child: item.imageUrl.isEmpty ? Container(color: _soft, child: Icon(_categoryIcon(item.category), size: 46, color: _brown)) : CachedNetworkImage(imageUrl: item.imageUrl, fit: BoxFit.cover))),
        ...
        const SizedBox(height: 10), _detailRow('Colour', item.colour), _detailRow('Category', item.category), _detailRow('Style', item.style),
        if (item.season.isNotEmpty && item.season != 'All seasons') _detailRow('Season', item.season),
        if (item.notes.isNotEmpty) ...[const SizedBox(height: 4), const Text('NOTES', style: TextStyle(color: _muted, fontSize: 10.5, fontWeight: FontWeight.w800, letterSpacing: .6)), const SizedBox(height: 6), Text(item.notes, style: const TextStyle(color: _text, height: 1.4))],
        const SizedBox(height: 20),
        Row(children: [
          ...
          const SizedBox(width: 10), Expanded(child: FilledButton.icon(onPressed: () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (_) => AIStylistScreen(selectedItem: item))); }, icon: const Icon(Icons.auto_awesome_rounded), label: const Text('Style this'), style: FilledButton.styleFrom(backgroundColor: _brown))),
        ]),
  ...
  Future<void> _showEditItem(WardrobeItem item, String uid) async {
    ...
    await showModalBottomSheet<void>(context: context, isScrollControlled: true, showDragHandle: true, backgroundColor: _cream, builder: (sheetContext) => StatefulBuilder(builder: (context, setSheetState) {
      Future<void> pickNewPhoto() async {
        final picked = await showModalBottomSheet<File?>(context: context, showDragHandle: true, backgroundColor: _cream, builder: (pickContext) => SafeArea(child: Padding(
          padding: const EdgeInsets.fromLTRB(20, 8, 20, 25), child: Row(children: [
            ...
            const SizedBox(width: 12), Expanded(child: _photoSourceTile(icon: Icons.photo_outlined, label: 'Gallery', onTap: () async { final image = await ImagePickerService.pickGallery(); if (!pickContext.mounted) return; Navigator.pop(pickContext, image); })),
          ]),
        )));
      ...
      return SafeArea(child: Padding(
        ...
        child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 15),
          GestureDetector(onTap: pickNewPhoto, child: Stack(children: [
