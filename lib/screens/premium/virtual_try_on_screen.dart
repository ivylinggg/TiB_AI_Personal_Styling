...
class _VirtualTryOnScreenState extends State<VirtualTryOnScreen> {
  ...
  Future<void> _load() async {
    ...
    if (uid == null) { if (mounted) setState(() { _loading = false; _status = 'Please sign in again before using Virtual Try-On.'; }); return; }
    try {
      final results = await Future.wait<dynamic>([TibModelService.load(), FirestoreService.getWardrobeItems(uid), StylePreferenceService.getStylePreferences(uid), FirestoreService.getLatestColourAnalysis(uid)]);
      if (!mounted || FirebaseAuth.instance.currentUser?.uid != uid) return;
      final prefs = results[2] as Map<String, dynamic>?;
      setState(() { _model = results[0] as TibModelProfile?; _wardrobe = results[1] as List<WardrobeItem>; _styles = List<String>.from(prefs?['styles'] ?? const <String>[]); _preferences = List<String>.from(prefs?['preferences'] ?? const <String>[]); _analysis = results[3] as ColourAnalysisResult?; _loading = false; });
    } catch (_) { if (mounted) setState(() { _loading = false; _status = 'Could not load your Personal TiB Model. Please try again.'; }); }
  }
  ...
  void _toggle(WardrobeItem item) {
    ...
    setState(() {
      ...
      else if (_selectedIds.length < 6) { _selectedIds.add(item.id); }
      else { _status = 'Choose up to 6 pieces for one look.'; return; }
      _generatedImageUrl = null; _requestId = null;
  ...
  Future<void> _letTiBStyleMe() async {
    ...
    } catch (_) { if (mounted) setState(() { _busy = false; _status = 'TiB could not build the look right now. You can choose the pieces yourself.'; }); }
  }
  ...
  Future<void> _generate({List<WardrobeItem>? items}) async {
    ...
    } catch (error) { if (mounted) setState(() { _busy = false; _status = 'Virtual Try-On failed: $error'; }); }
  }
...
Create one photorealistic head-to-toe image of this same user wearing only the selected real wardrobe pieces. Preserve garment construction, colour, texture and details. Show the complete body whenever possible, including shoes when selected. Use a natural standing pose and realistic fabric fit.''';
  ...
  Widget _identityHero(TibModelProfile? model) {
    final complete = model?.isComplete == true; final bodyFile = model?.bodyFile;
  ...
  Widget _modelCard(TibModelProfile? model) { final complete=model?.isComplete==true; return Container(padding:const EdgeInsets.all(16),decoration:BoxDecoration(color:AppColors.surface,borderRadius:BorderRadius.circular(24),border:Border.all(color:AppColors.border)),child:Column(children:[Row(children:[const Icon(Icons.straighten_rounded,color:AppColors.primary,size:20),const SizedBox(width:8),const Expanded(child:Text('REAL BODY PROFILE',style:TextStyle(fontSize:11,fontWeight:FontWeight.w800,letterSpacing:1))),_pill(complete?'READY':'INCOMPLETE',complete)]),const SizedBox(height:12),if(model==null||!complete)const Align(alignment:Alignment.centerLeft,child:Text('Face + full-body reference + height + weight + bust + waist + hips are required.',style:TextStyle(fontSize:12,color:AppColors.textSecondary,height:1.4))) else ...[Row(children:[_metric('HEIGHT','${model.height.toStringAsFixed(0)} cm'),_metric('BUST','${model.bust.toStringAsFixed(1)} cm'),_metric('WAIST','${model.waist.toStringAsFixed(1)} cm'),_metric('HIPS','${model.hips.toStringAsFixed(1)} cm')]),const SizedBox(height:10),Row(children:[_tag(Icons.person_outline_rounded,model.bodyShape),const SizedBox(width:7),_tag(Icons.face_retouching_natural_rounded,model.faceShape),const Spacer(),Text('${model.weight.toStringAsFixed(1)} kg',style:const TextStyle(fontSize:11,color:AppColors.textSecondary,fontWeight:FontWeight.w700))])]])); }
