...
class _VirtualTryOnScreenState extends State<VirtualTryOnScreen> {
  ...
  Widget _identityHero(TibModelProfile? model) {
    ...
    return Container(
      ...
      child: Row(children: [
        Container(width: 66, height: 82, decoration: BoxDecoration(borderRadius: BorderRadius.circular(22), color: AppColors.lavenderMist), clipBehavior: Clip.antiAlias, child: bodyFile != null && bodyFile.existsSync() ? Image.file(bodyFile, fit: BoxFit.cover) : const Icon(Icons.person_rounded, size: 38, color: AppColors.primary)),
  ...
  Widget _modelCard(TibModelProfile? model) {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        else ...[_metrics(model), const SizedBox(height: 10), Row(children: [_tag(Icons.person_outline_rounded, model.bodyShape), const SizedBox(width: 7), _tag(Icons.face_retouching_natural_rounded, model.faceShape), const Spacer(), Text('${model.weight.toStringAsFixed(1)} kg', style: const TextStyle(fontSize: 11, color: AppColors.textSecondary, fontWeight: FontWeight.w700))])],
      ]),
    );
  }
  ...
  Widget _tag(IconData icon, String text) => Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 6), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 13, color: AppColors.primary), const SizedBox(width: 5), Text(text, style: const TextStyle(fontSize: 9, fontWeight: FontWeight.w800))]));

  Widget _pill(String text, bool ok) => Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5), decoration: BoxDecoration(color: ok ? AppColors.lavenderMist : Colors.white, borderRadius: BorderRadius.circular(12)), child: Text(text, style: const TextStyle(fontSize: 9, fontWeight: FontWeight.w900)));
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Wrap(spacing: 7, runSpacing: 7, children: [
            for (final value in _occasions)
              ChoiceChip(label: Text(value), selected: _occasion == value, onSelected: _busy ? null : (_) => setState(() { _occasion = value; _generatedImageUrl = null; _status = ''; })),
          ]),
        ]),
      );

  Widget _modeActions() => Row(children: [Expanded(child: FilledButton.icon(onPressed: _busy ? null : _letTiBStyleMe, icon: const Icon(Icons.auto_awesome_rounded), label: Text(_busy ? 'Working…' : 'Let TiB Style Me'))]);

  Widget _wardrobeSection(List<WardrobeItem> selected) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    const Text('YOUR WARDROBE', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1.2, color: AppColors.textMuted)),
    const SizedBox(height: 10),
    if (_wardrobe.isEmpty)
      const Text('No wardrobe pieces yet.', style: TextStyle(color: AppColors.textSecondary))
    else
      SizedBox(height: 210, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: _wardrobe.length, separatorBuilder: (_, __) => const SizedBox(width: 10), itemBuilder: (_, index) {
        final item = _wardrobe[index];
        final chosen = selected.any((entry) => entry.id == item.id);
        return SizedBox(width: 145, child: InkWell(onTap: () => _toggle(item), borderRadius: BorderRadius.circular(18), child: Container(decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: chosen ? AppColors.primary : AppColors.border, width: chosen ? 2 : 1)), clipBehavior: Clip.antiAlias, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: item.imageUrl.isEmpty ? const Center(child: Icon(Icons.checkroom_outlined, size: 36, color: AppColors.textMuted)) : CachedNetworkImage(imageUrl: item.imageUrl, fit: BoxFit.cover, width: double.infinity)), Padding(padding: const EdgeInsets.fromLTRB(10, 8, 10, 10), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(item.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w800)), const SizedBox(height: 2), Text(item.category, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 9.5, color: AppColors.textMuted))]))]))));
      })),
  ]);

  Widget _selectedBar(List<WardrobeItem> selected) => Container(padding: const EdgeInsets.all(13), decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(18), border: Border.all(color: AppColors.border)), child: Wrap(spacing: 7, runSpacing: 7, children: [for (final item in selected) Chip(label: Text(item.name), onDeleted: _busy ? null : () => _toggle(item))]));

  Widget _statusCard() => Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(18), border: Border.all(color: AppColors.border)), child: Text(_status, style: const TextStyle(fontSize: 11.5, color: AppColors.textSecondary, height: 1.45)));
