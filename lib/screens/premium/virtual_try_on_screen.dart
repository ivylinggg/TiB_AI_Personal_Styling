...
class _VirtualTryOnScreenState extends State<VirtualTryOnScreen> {
  ...
  Future<void> _letTiBStyleMe() async {
    ...
    final uid = FirebaseAuth.instance.currentUser?.uid;
    if (uid == null) { setState(() => _status = 'Please sign in again.'); return; }
    ...
    try {
      ...
      if (mounted) setState(() { _busy = false; _status = 'TiB could not build the look right now. You can choose the pieces yourself.'; });
    }
  }
  ...
  Widget _occasionSection() => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Wrap(spacing: 7, runSpacing: 7, children: _occasions.map((value) => ChoiceChip(label: Text(value), selected: _occasion == value, onSelected: _busy ? null : (_) => setState(() { _occasion = value; _generatedImageUrl = null; _selectedIds.clear(); _status = ''; }))).toList()),
        ]),
      );
  ...
  Widget _wardrobeSection(List<WardrobeItem> selected) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (_wardrobe.isEmpty)
          Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(18), border: Border.all(color: AppColors.border)), child: const Text('Add real wardrobe pieces before using Virtual Try-On.'))
        else
          GridView.builder(
            shrinkWrap: true,
            ...
            itemBuilder: (_, index) {
              ...
              final active = _selectedIds.contains(item.id);
              return InkWell(
                onTap: () => _toggle(item),
                borderRadius: BorderRadius.circular(16),
                child: Container(
                  decoration: BoxDecoration(color: active ? AppColors.primarySoft : AppColors.surface, borderRadius: BorderRadius.circular(16), border: Border.all(color: active ? AppColors.primary : AppColors.border, width: active ? 1.5 : 1)),
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Padding(padding: const EdgeInsets.all(7), child: Text(item.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 9.5, fontWeight: FontWeight.w700))),
                    if (active) const Padding(padding: EdgeInsets.fromLTRB(7, 0, 7, 7), child: Icon(Icons.check_circle_rounded, size: 16, color: AppColors.primary)),
                  ]),
                ),
              );
            },
  ...
  Widget _generatedCard() => Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(22), border: Border.all(color: AppColors.border)),
        child: Column(children: [ClipRRect(borderRadius: BorderRadius.circular(18), child: Image.network(_generatedImageUrl!, fit: BoxFit.cover, width: double.infinity)), const SizedBox(height: 10), Row(children: [const Icon(Icons.auto_awesome_rounded, color: AppColors.primary), const SizedBox(width: 8), const Expanded(child: Text('Your Virtual You', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800))), IconButton(onPressed: _busy ? null : _generate, icon: const Icon(Icons.refresh_rounded))])]),
      );
}
