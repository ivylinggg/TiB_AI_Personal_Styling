  ...
  Future<void> _createAvatarLook() async {
    ...
    try {
      ...
      if (recommendation == null) {
        setState(() { _busy = false; _status = 'TiB could not find a complete match. Try adding more wardrobe pieces.'; });
        return;
      }
      ...
      if (selected.isEmpty) {
        setState(() { _busy = false; _recommendation = recommendation; _status = 'TiB found styling guidance, but no wardrobe match was returned.'; });
        return;
      }
...
5. Head-to-toe composition.''';
  ...
  Future<void> _saveLook() async {
    ...
    try {
      await FirestoreService.saveOutfitLook(uid: uid, occasion: _occasion, itemIds: _look.map((item) => item.id).toList(growable: false), matchScore: _recommendation?.matchScore ?? 0, season: _analysis?.season ?? 'Unknown', title: _recommendation?.displayTitle, notes: _recommendation?.stylingNotes.join(' • '));
    ...
    return Container(
      ...
      child: Column(children: [
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          Container(width: 92, height: 112, decoration: BoxDecoration(color: Colors.white.withValues(alpha: .58), borderRadius: BorderRadius.circular(24), border: Border.all(color: Colors.white.withValues(alpha: .7))), clipBehavior: Clip.antiAlias, child: face != null && face.existsSync() ? Image.file(face, fit: BoxFit.cover) : const Icon(Icons.face_retouching_natural_rounded, size: 44, color: AppColors.primary)),
          const SizedBox(width: 10),
          Container(width: 92, height: 112, decoration: BoxDecoration(color: Colors.white.withValues(alpha: .58), borderRadius: BorderRadius.circular(24), border: Border.all(color: Colors.white.withValues(alpha: .7))), clipBehavior: Clip.antiAlias, child: body != null && body.existsSync() ? Image.file(body, fit: BoxFit.cover) : const Icon(Icons.accessibility_new_rounded, size: 44, color: AppColors.primary)),
        ]),
  ...
  Widget _buildOccasion() => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Wrap(spacing: 7, runSpacing: 7, children: _occasions.map((value) => ChoiceChip(label: Text(value), selected: _occasion == value, onSelected: _busy ? null : (_) => setState(() { _occasion = value; _imageUrl = null; _look = const []; _recommendation = null; _status = ''; }))).toList()),
        ]),
      );
  ...
  Widget _buildResult() => Container(
        ...
        decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(22), border: Border.all(color: AppColors.border)),
        child: Column(children: [
          if (_imageUrl != null) ClipRRect(borderRadius: BorderRadius.circular(18), child: Image.network(_imageUrl!, fit: BoxFit.cover, width: double.infinity)),
          const SizedBox(height: 12),
          Row(children: [Expanded(child: Text(_recommendation?.displayTitle ?? 'Your virtual look', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800))), IconButton(onPressed: _saveLook, icon: const Icon(Icons.bookmark_border_rounded))]),
          if (_recommendation?.explanation.isNotEmpty == true) ...[const SizedBox(height: 7), Text(_recommendation!.explanation, style: const TextStyle(color: AppColors.textSecondary, fontSize: 11.5, height: 1.4))],
        ]),
      );
}
