...
class _AIVirtualStylingStudioScreenState extends State<AIVirtualStylingStudioScreen> {
          ...
          children: [
            Row(
              children: [
                ...
                const Expanded(child: Text('AI Styling Studio', textAlign: TextAlign.center, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800))),
            ...
            _actionCard(icon: Icons.face_retouching_natural_rounded, title: ready ? 'Edit My TiB Model' : 'Create My TiB Model', subtitle: ready ? 'Update your face, body photo or model details.' : 'Build your reusable face and body profile.', onTap: () => _openModelSetup(context), highlight: !ready),
            const SizedBox(height: 10),
            _actionCard(icon: Icons.checkroom_rounded, title: 'Build My Look', subtitle: 'Pick each piece from your own wardrobe.', onTap: () => _openBuilder(context), enabled: ready),
            const SizedBox(height: 10),
            _actionCard(icon: Icons.auto_awesome_rounded, title: 'Let TiB Style Me', subtitle: 'AI chooses a complete look for your occasion.', onTap: () => _openAiStylist(context), enabled: ready, highlight: true),
            const SizedBox(height: 10),
            _actionCard(icon: Icons.view_in_ar_rounded, title: 'AI Virtual Try-On', subtitle: 'Select your wardrobe and generate a try-on image with your TiB Model.', onTap: () => _openTryOn(context), enabled: ready, highlight: true),
            const SizedBox(height: 18),
            Container(
  ...
  Widget _buildModelHero(TibModelProfile? model, bool ready) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(gradient: AppGradients.premium, borderRadius: BorderRadius.circular(32)),
      child: Row(
        children: [
          Container(width: 92, height: 112, decoration: BoxDecoration(color: Colors.white.withValues(alpha: .55), borderRadius: BorderRadius.circular(28)), clipBehavior: Clip.antiAlias, child: photo != null && photo.existsSync() ? Image.file(photo, fit: BoxFit.cover) : const Icon(Icons.person_rounded, size: 48, color: AppColors.primary)),
          const SizedBox(width: 17),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5), decoration: BoxDecoration(color: Colors.white.withValues(alpha: .72), borderRadius: BorderRadius.circular(20)), child: Text(ready ? 'MODEL READY' : 'MODEL NOT SET', style: const TextStyle(color: AppColors.primaryDark, fontSize: 8, fontWeight: FontWeight.w900, letterSpacing: .9))),
            const SizedBox(height: 12),
            Text(ready ? 'This is your\nTiB Model.' : 'Create your\nTiB Model.', style: const TextStyle(fontSize: 24, height: 1.0, fontWeight: FontWeight.w900)),
            const SizedBox(height: 8),
            Text(ready ? 'Ready to style with your own wardrobe.' : 'Add your face once, then build looks around you.', style: const TextStyle(color: AppColors.textSecondary, fontSize: 11, height: 1.4)),
          ])),
        ],
      ),
    );
  ...
  Widget _buildModelProfile(TibModelProfile? model, bool ready) {
    ...
    return Container(
      ...
      child: Column(children: [
        Row(children: [const Icon(Icons.face_retouching_natural_rounded, size: 18, color: AppColors.primary), const SizedBox(width: 9), const Expanded(child: Text('Face Shape', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800))), _valueBadge(model.faceShape, model.faceShape != 'Not scanned')]),
        const Divider(height: 22),
        Row(children: [const Icon(Icons.accessibility_new_rounded, size: 18, color: AppColors.primary), const SizedBox(width: 9), const Expanded(child: Text('Body Shape', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800))), _valueBadge(model.bodyShape, model.bodyShape != 'Not measured')]),
        const SizedBox(height: 16),
        const Align(alignment: Alignment.centerLeft, child: Text('BODY MEASUREMENTS', style: TextStyle(fontSize: 9, letterSpacing: 1.2, fontWeight: FontWeight.w900, color: AppColors.textMuted))),
        const SizedBox(height: 10),
        ...
        if (ready) ...[
          ...
          Container(width: double.infinity, padding: const EdgeInsets.all(11), decoration: BoxDecoration(color: AppColors.primarySoft.withValues(alpha: .45), borderRadius: BorderRadius.circular(14)), child: const Row(children: [Icon(Icons.auto_awesome_rounded, size: 15, color: AppColors.primaryDark), SizedBox(width: 7), Expanded(child: Text('Your face shape and body shape are automatically used for personalised AI styling.', style: TextStyle(color: AppColors.primaryDark, fontSize: 9.5, height: 1.35)))])),
        ],
      ),
    );
  }
