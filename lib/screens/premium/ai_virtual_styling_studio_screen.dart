...
class _AIVirtualStylingStudioScreenState extends State<AIVirtualStylingStudioScreen> {
  ...
  Widget _buildModelProfile(TibModelProfile? model, bool ready) {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        Align(alignment: Alignment.centerLeft, child: Text('BODY MEASUREMENTS', style: const TextStyle(fontSize: 9, letterSpacing: 1.2, fontWeight: FontWeight.w900, color: AppColors.textMuted))),
        const SizedBox(height: 10),
  ...
  Widget _measurement(String label, double value, String unit) => Expanded(child: Container(margin: const EdgeInsets.symmetric(horizontal: 3), padding: const EdgeInsets.symmetric(vertical: 11, horizontal: 8), decoration: BoxDecoration(color: AppColors.surfaceMuted, borderRadius: BorderRadius.circular(14)), child: Column(children: [Text(label, style: const TextStyle(fontSize: 9, color: AppColors.textMuted, fontWeight: FontWeight.w700)), const SizedBox(height: 4), Text(value > 0 ? '${value == value.roundToDouble() ? value.toInt() : value.toStringAsFixed(1)} $unit' : '—', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w900))])));
