...
class _DashboardDesignedScreenState extends State<DashboardDesignedScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final savedChallengeId = (user?['dailyChallengeId'] as num?)?.toInt();
      ...
      setState(() {
        ...
        _digitalChallengeCount = history.where((entry) => entry['category'] == 'Digital Etiquette').length;
      });
    } catch (_) {
  ...
  Map<String, int> _scoreBreakdown(ColourAnalysisResult? result) {
    ...
    final appearance = (20 + paletteMatchPoints + wardrobePoints).clamp(0, 40);
    ...
    final communication = (8 + (styles * 2) + (preferences * 2)).clamp(0, 20);
  ...
  int _styleScore(ColourAnalysisResult? result) {
    return _scoreBreakdown(result).values.fold(0, (sum, value) => sum + value).clamp(0, 100);
  }
  ...
  Future<void> _showStyleScoreDetails(ColourAnalysisResult? result) async {
    ...
    final total = breakdown.values.fold(0, (sum, value) => sum + value).clamp(0, 100);
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    const Expanded(
                      child: Text('Your Style Score', style: TextStyle(fontSize: 23, fontWeight: FontWeight.w900)),
                    ),
                    Text('$total/100', style: const TextStyle(color: AppColors.primaryDark, fontSize: 17, fontWeight: FontWeight.w900)),
                  ],
                ),
                ...
                Container(
                  ...
                  decoration: BoxDecoration(color: AppColors.lavenderMist, borderRadius: BorderRadius.circular(18)),
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Icon(Icons.lightbulb_outline_rounded, color: AppColors.primaryDark),
                      const SizedBox(width: 10),
                      Expanded(child: Text(_scoreNextStep(breakdown), style: const TextStyle(fontSize: 12, height: 1.4, fontWeight: FontWeight.w600))),
                    ],
                  ),
                ),
              ],
  ...
  List<Widget> _scoreDetailRows(Map<String, int> breakdown) {
    ...
    const descriptions = {
      'Appearance': 'Colour harmony, wardrobe readiness and palette matches.',
      'Behavior': 'Consistency with confidence and professional daily habits.',
      'Communication': 'Your saved style direction and personal preferences.',
      'Digital Etiquette': 'Professional digital presence and digital habits.',
    };
      ...
      return Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Expanded(child: Text(entry.key, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w800))),
                Text('${entry.value}/$max', style: const TextStyle(color: AppColors.primaryDark, fontSize: 12, fontWeight: FontWeight.w800)),
              ],
            ),
            const SizedBox(height: 5),
            ClipRRect(
              borderRadius: BorderRadius.circular(99),
              child: LinearProgressIndicator(value: progress, minHeight: 7, backgroundColor: AppColors.border, color: AppColors.primary),
            ),
            const SizedBox(height: 5),
            Text(descriptions[entry.key] ?? '', style: const TextStyle(color: AppColors.textSecondary, fontSize: 10.5, height: 1.35)),
          ],
        ),
      );
    }).toList();
  ...
  Future<void> _completeChallenge() async {
    ...
    try {
      await FirebaseFirestore.instance.collection('users').doc(uid).set({
        ...
        'dailyChallengeHistory': FieldValue.arrayUnion([entry]),
      }, SetOptions(merge: true));

      if (!mounted) return;
      final updatedHistory = [..._challengeHistory, {
        'date': dateKey,
        'challengeId': challengeId,
        'title': challenge.title,
        'category': challenge.category,
      ...
      setState(() {
        ...
        _challengeHistory = updatedHistory;
        _challengeStreak = _calculateStreak(updatedHistory, now);
        _digitalChallengeCount = updatedHistory.where((item) => item['category'] == 'Digital Etiquette').length;
      });
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Nice work — ${challenge.title} is complete! +10 XP ✨')),
      );
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Container(width: 46, height: 46, decoration: const BoxDecoration(color: AppColors.lavenderMist, shape: BoxShape.circle), child: Icon(challenge.icon, color: AppColors.primaryDark)),
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Expanded(child: Text(challenge.howTo, style: const TextStyle(fontSize: 11.5, height: 1.4, fontWeight: FontWeight.w600))),
                    ],
                  ),
                ),
                const SizedBox(height: 15),
                if (result == null)
                  const Text('Complete your colour analysis first to personalise this challenge.', style: TextStyle(color: AppColors.textMuted, fontSize: 10.5))
                else
                  SizedBox(
                    ...
                    child: FilledButton.icon(
                      ...
                      icon: Icon(_challengeCompleted ? Icons.check_rounded : Icons.auto_awesome_rounded),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 1),
              Text(result == null ? 'Let’s make today feel more like you.' : '${result.season} • ${result.undertone}', style: const TextStyle(color: AppColors.textSecondary, fontSize: 11)),
            ],
          ),
        ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  Row(children: result.colours.take(7).map((name) => Padding(padding: const EdgeInsets.only(right: 5), child: ColourSwatch(name: name, size: 25))).toList()),
                ],
              ),
            ),
    ...
    return _card(
      ...
      child: Container(
        ...
        child: result == null
            ? Row(
                children: [
                  Container(width: 50, height: 50, decoration: const BoxDecoration(color: AppColors.primarySoft, shape: BoxShape.circle), child: const Icon(Icons.palette_outlined, color: AppColors.primaryDark)),
                  const SizedBox(width: 11),
                  const Expanded(
            ...
            : Row(
                ...
                children: [
                  ColourSwatch(name: name, size: 58, showLabel: false),
                  ...
                  Expanded(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        ...
                        const Spacer(),
                        Text('${date.day}/${date.month}', style: const TextStyle(color: AppColors.textMuted, fontSize: 9.5, fontWeight: FontWeight.w700)),
                      ]),
                      const SizedBox(height: 3),
                      Text(name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
                      const SizedBox(height: 4),
                      Text('From your personal best-colour palette', style: const TextStyle(color: AppColors.primaryDark, fontSize: 10, fontWeight: FontWeight.w700)),
                      const SizedBox(height: 4),
  ...
  Widget _styleScoreCard(ColourAnalysisResult? result) {
    ...
    final breakdown = _scoreBreakdown(result);
    final appearance = breakdown['Appearance'] ?? 0;
    final behaviour = breakdown['Behavior'] ?? 0;
    final communication = breakdown['Communication'] ?? 0;
    final digital = breakdown['Digital Etiquette'] ?? 0;

    return _card(
      onTap: () => _showStyleScoreDetails(result),
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
            ...
            const Padding(padding: EdgeInsets.only(left: 3, bottom: 2), child: Text('/100', style: TextStyle(color: AppColors.textMuted, fontSize: 11, fontWeight: FontWeight.w700))),
            const Spacer(),
          ...
          const SizedBox(height: 10),
          Row(children: [
            _ScoreMini(label: 'A', value: appearance, max: 40),
            const SizedBox(width: 5),
            _ScoreMini(label: 'B', value: behaviour, max: 25),
            const SizedBox(width: 5),
            _ScoreMini(label: 'C', value: communication, max: 20),
            const SizedBox(width: 5),
            _ScoreMini(label: 'D', value: digital, max: 15),
          ]),
          const SizedBox(height: 7),
  ...
  Widget _challenge(ColourAnalysisResult? result) {
    final challenge = _dailyChallenges[_challengeId(DateTime.now())];
    return InkWell(
      onTap: () => _showChallengeDetails(result),
      borderRadius: BorderRadius.circular(19),
      child: Container(
        padding: const EdgeInsets.fromLTRB(15, 14, 15, 14),
        decoration: BoxDecoration(gradient: const LinearGradient(colors: [AppColors.lavenderMist, AppColors.background], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(19), border: Border.all(color: AppColors.border)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            if (_challengeStreak > 0) ...[
              const Icon(Icons.local_fire_department_rounded, color: AppColors.premiumAccent, size: 16),
              const SizedBox(width: 2),
              Text('$_challengeStreak day', style: const TextStyle(color: AppColors.primaryDark, fontSize: 9.5, fontWeight: FontWeight.w800)),
            ],
          ]),
          const SizedBox(height: 10),
          Row(children: [
            Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(AppRadius.full)), child: Text(challenge.category, style: const TextStyle(color: AppColors.primaryDark, fontSize: 9, fontWeight: FontWeight.w800))),
            const Spacer(),
            const Text('+10 XP', style: TextStyle(color: AppColors.textMuted, fontSize: 9.5, fontWeight: FontWeight.w800)),
          ]),
          const SizedBox(height: 8),
          Text(challenge.title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900)),
          const SizedBox(height: 4),
          Text(challenge.description, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: AppColors.textSecondary, fontSize: 11, height: 1.38)),
          const SizedBox(height: 11),
          SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: result == null || _challengeCompleted ? null : _completeChallenge, icon: Icon(_challengeCompleted ? Icons.check_rounded : Icons.auto_awesome_rounded, size: 17), label: Text(_challengeCompleted ? 'Completed today' : 'Accept today\'s challenge'), style: FilledButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: Colors.white, disabledBackgroundColor: AppColors.primarySoft, disabledForegroundColor: AppColors.primaryDark, minimumSize: const Size.fromHeight(44), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(13))))),
    ...
    return _card(
      ...
      child: Padding(
        ...
        child: Row(children: [
          ...
          const SizedBox(width: 10),
          const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Recent Report', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800)), SizedBox(height: 3), Text('Your latest colour analysis report', style: TextStyle(color: AppColors.textSecondary, fontSize: 10.5))])),
  ...
  Widget _wardrobePreview() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [const Expanded(child: Text('My Wardrobe', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800))), TextButton(onPressed: () => _open(const WardrobeScreen()), child: const Text('View all'))]),
      const SizedBox(height: 3),
...
class _HeroPill extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), decoration: BoxDecoration(color: Colors.white.withValues(alpha: .16), borderRadius: BorderRadius.circular(AppRadius.full)), child: Text(label, style: const TextStyle(color: Colors.white, fontSize: 10.5, fontWeight: FontWeight.w800)));
  }
}
...
class _MiniTag extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 4), decoration: BoxDecoration(color: AppColors.eggYolk, borderRadius: BorderRadius.circular(AppRadius.full)), child: Text(label, style: const TextStyle(color: AppColors.primaryDark, fontSize: 8, fontWeight: FontWeight.w800)));
  }
}
