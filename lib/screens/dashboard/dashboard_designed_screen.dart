...
class _DashboardDesignedScreenState extends State<DashboardDesignedScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final history = rawHistory is List
          ? rawHistory.whereType<Map>().map((entry) => Map<String, dynamic>.from(entry)).toList()
          : <Map<String, dynamic>>[];
  ...
  int _calculateStreak(List<Map<String, dynamic>> history, DateTime now) {
    final dates = history.map((entry) => entry['date']).whereType<String>().toSet();
  ...
  int _styleScore(ColourAnalysisResult? result) => _scoreBreakdown(result).values.fold<int>(0, (total, value) => total + value).clamp(0, 100).toInt();
  ...
  String _scoreNextStep(Map<String, int> breakdown) {
    ...
    switch (lowest) {
      case 'Appearance': return 'Add wardrobe pieces that match your personal palette and complete your colour analysis.';
      case 'Behavior': return 'Complete today\'s challenge and build a daily confidence habit.';
      case 'Communication': return 'Set your style preferences so TiB can understand how you like to present yourself.';
      default: return 'Complete a digital-presence challenge and keep your profile image professional and consistent.';
    }
  }

  Future<void> _showStyleScoreDetails(ColourAnalysisResult? result) async {
    final breakdown = _scoreBreakdown(result);
    final total = breakdown.values.fold<int>(0, (sum, value) => sum + value).clamp(0, 100).toInt();
      ...
      builder: (sheetContext) => SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              const Expanded(child: Text('Your Style Score', style: TextStyle(fontSize: 23, fontWeight: FontWeight.w900))),
    ...
    return breakdown.entries.map((entry) {
      ...
      return Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ClipRRect(borderRadius: BorderRadius.circular(99), child: LinearProgressIndicator(value: progress, minHeight: 7, backgroundColor: AppColors.border, color: AppColors.primary)),
          const SizedBox(height: 5),
  ...
  Future<void> _completeChallenge() async {
    ...
    try {
      ...
      if (!mounted) return;
      final updatedHistory = [..._challengeHistory, entry];
      setState(() {
            ...
            if (result == null)
              ...
              SizedBox(
                ...
                child: FilledButton.icon(
                  onPressed: _challengeCompleted ? null : () { Navigator.pop(sheetContext); _completeChallenge(); },
          ...
          child: ListView(
            ...
            children: [
              ...
              Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(width: 10),
                SizedBox(width: 160, child: _styleScoreCard(result)),
              ]),
              const SizedBox(height: 12),
              _challenge(result),
              const SizedBox(height: 12),
  ...
  Widget _header(ColourAnalysisResult? result) {
    ...
    return Row(children: [
      CircleAvatar(radius: 25, backgroundColor: AppColors.primarySoft, backgroundImage: _photoUrl?.isNotEmpty == true ? CachedNetworkImageProvider(_photoUrl!) : null, child: _photoUrl?.isNotEmpty == true ? null : const Icon(Icons.person_rounded, color: AppColors.primaryDark)),
      const SizedBox(width: 11),
  ...
  Widget _seasonHero(ColourAnalysisResult? result) {
    if (result == null) {
      return _card(onTap: () => _open(const AnalysisScreen()), child: Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(gradient: AppGradients.primary, borderRadius: BorderRadius.circular(22)), child: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
