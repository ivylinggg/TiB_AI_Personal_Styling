...
class _CustomerForumScreenState extends State<CustomerForumScreen> {
  ...
  Future<void> _createPost() async {
    ...
    try {
          ...
          builder: (dialogBuildContext, setDialogState) => AlertDialog(
            ...
            content: SingleChildScrollView(
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                TextField(controller: titleController, textInputAction: TextInputAction.next, decoration: const InputDecoration(labelText: 'Title', hintText: 'What do you want to discuss?')),
                const SizedBox(height: 12),
                DropdownButtonFormField<String>(
                  initialValue: category,
                  decoration: const InputDecoration(labelText: 'Category'),
                  items: _categories.where((item) => item != 'All').map((item) => DropdownMenuItem(value: item, child: Text(item))).toList(),
            ...
            actions: [
              TextButton(onPressed: saving ? null : () => Navigator.of(dialogContext).pop(false), child: const Text('Cancel')),
              FilledButton(
      ...
      if (result == true && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Your post is now visible to the TiB community.')));
      }
    } finally {
  ...
  Future<void> _toggleLike(DocumentReference<Map<String, dynamic>> postRef) async {
    ...
    try {
      ...
      final current = (post.data()?['likeCount'] as num?)?.toInt() ?? 0;
      if (existing.exists) {
  ...
  Widget _postCard(QueryDocumentSnapshot<Map<String, dynamic>> document) {
    ...
    final replies = (data['commentCount'] as num?)?.toInt() ?? 0;
    return Card(
      elevation: 0,
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              if (official) const Padding(padding: EdgeInsets.only(left: 7), child: Icon(Icons.verified_rounded, size: 15, color: AppColors.primary)),
              const Spacer(),
            ...
            Row(children: [
              ...
              const SizedBox(width: 4),
              Text('$replies', style: const TextStyle(fontSize: 10.5, color: AppColors.textSecondary)),
            ]),
          ]),
        ),
      ),
        ...
        builder: (context, snapshot) {
          ...
          return ListView(
            ...
            children: [
              ...
              Row(children: [const Text('COMMUNITY POSTS', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1.3, color: AppColors.textSecondary)), const Spacer(), Text('${visible.length} posts', style: const TextStyle(fontSize: 11, color: AppColors.textMuted))]),
              const SizedBox(height: 10),
              if (snapshot.connectionState == ConnectionState.waiting && documents.isEmpty)
                const Padding(padding: EdgeInsets.all(40), child: Center(child: CircularProgressIndicator()))
              else if (visible.isEmpty)
                const Padding(padding: EdgeInsets.fromLTRB(0, 30, 0, 40), child: Center(child: Text('No posts match your search yet.')))
              else
...
class _ForumPostDetailScreenState extends State<ForumPostDetailScreen> {
    ...
    try {
      ...
      if (success) {
        ...
        if (mounted && _scrollController.hasClients) {
          await _scrollController.animateTo(_scrollController.position.maxScrollExtent, duration: const Duration(milliseconds: 280), curve: Curves.easeOut);
        }
      }
    } finally {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, commentsSnapshot) {
          final post = widget.initialData;
          final comments = [...?commentsSnapshot.data?.docs];
          comments.sort((a, b) => _date(a.data()['createdAt']).compareTo(_date(b.data()['createdAt'])));

          if (commentsSnapshot.hasError) {
            return Column(children: [const Expanded(child: Center(child: Text('Unable to load replies.'))), _replyBar(theme)]);
          }
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    if (commentsSnapshot.connectionState == ConnectionState.waiting && comments.isEmpty)
                      ...
                      ...comments.asMap().entries.map((entry) => _replyCard(entry.value.data(), entry.key == comments.length - 1)),
                  ],
                ),
              ),
              _replyBar(theme),
  ...
  Widget _postCard(Map<String, dynamic> data) {
    final official = data['isOfficial'] == true;
    return Card(
      elevation: 0,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(22), side: BorderSide(color: official ? AppColors.primarySoft : AppColors.border)),
      child: Padding(
        padding: const EdgeInsets.all(18),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5), decoration: BoxDecoration(color: official ? AppColors.primarySoft : AppColors.surfaceMuted, borderRadius: BorderRadius.circular(10)), child: Text(official ? 'TiB Team' : (data['category'] as String? ?? 'General'), style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: AppColors.primaryDark))),
            if (official) const Padding(padding: EdgeInsets.only(left: 6), child: Icon(Icons.verified_rounded, size: 16, color: AppColors.primary)),
          ]),
          const SizedBox(height: 12),
          Text(data['title'] as String? ?? 'Forum post', style: const TextStyle(fontSize: 25, fontWeight: FontWeight.w900, height: 1.1)),
          const SizedBox(height: 7),
          Text('Posted by ${data['authorName'] as String? ?? 'TiB User'} · ${_relativeDate(data['createdAt'])}', style: const TextStyle(color: AppColors.textSecondary, fontSize: 11)),
          const SizedBox(height: 16),
          Text(data['body'] as String? ?? '', style: const TextStyle(fontSize: 14, height: 1.6)),
          const SizedBox(height: 16),
          Row(children: [OutlinedButton.icon(onPressed: widget.onToggleLike, icon: const Icon(Icons.favorite_border_rounded), label: Text('Like ${(data['likeCount'] as num?)?.toInt() ?? 0}')), const SizedBox(width: 10), Text('${data['commentCount'] ?? 0} replies', style: const TextStyle(color: AppColors.textSecondary, fontSize: 12))]),
        ]),
      ),
    );
  }
  ...
  Widget _replyCard(Map<String, dynamic> data, bool last) {
    return Container(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        CircleAvatar(radius: 19, backgroundColor: AppColors.secondary, child: const Icon(Icons.person_outline_rounded, size: 19, color: AppColors.primary)),
        const SizedBox(width: 10),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [Expanded(child: Text(data['authorName'] as String? ?? 'TiB User', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800))), const SizedBox(width: 8), Text(_relativeDate(data['createdAt']), style: const TextStyle(fontSize: 9.5, color: AppColors.textMuted))]),
          const SizedBox(height: 6),
          Text(data['body'] as String? ?? '', style: const TextStyle(fontSize: 13, height: 1.45)),
        ])),
      ]),
    );
  }

  Widget _emptyReplies() {
    return Container(
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(color: AppColors.surfaceMuted, borderRadius: BorderRadius.circular(18)),
      child: const Column(children: [Icon(Icons.forum_outlined, size: 30, color: AppColors.primary), SizedBox(height: 8), Text('No replies yet.', style: TextStyle(fontWeight: FontWeight.w700)), SizedBox(height: 4), Text('Be the first to join the conversation.', style: TextStyle(color: AppColors.textSecondary, fontSize: 12))]),
    );
  }
      ...
      child: SafeArea(
        ...
        child: Padding(
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Expanded(child: TextField(controller: _commentController, focusNode: _commentFocusNode, minLines: 1, maxLines: 4, textInputAction: TextInputAction.send, onSubmitted: (_) => _send(), decoration: const InputDecoration(hintText: 'Write a reply...'))),
            const SizedBox(width: 8),
            IconButton.filled(onPressed: _sending ? null : _send, tooltip: 'Send reply', icon: _sending ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.send_rounded)),
          ]),
        ),
      ),
    );
