...
class _AnalysisResultScreenState extends State<AnalysisResultScreen> {
            ...
            SliverPadding(
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                  ...
                  _sectionHeading(
                    'Your Best Colours',
                    'The shades that make the easiest starting point for your wardrobe.',
                  ),
      ...
      return Container(
        ...
        child: const Row(
          children: [
            ...
            Expanded(
              child: Text(
                'Connecting your colour profile to your style space…',
        ...
        children: [
              ...
              const Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 3),
                    Text(
                      'Your colour result is now connected to the rest of your VYEA profile.',
          ...
          if (_styles.isNotEmpty)
            Wrap(
              spacing: 6,
              runSpacing: 6,
              children: _styles.take(4).map((style) => _miniChip(style, accent)).toList(),
            ),
          if (_preferences.isNotEmpty) ...[
            ...
            Wrap(
              spacing: 6,
              runSpacing: 6,
              children: _preferences.take(3).map((pref) => _miniChip(pref, accent, subtle: true)).toList(),
            ),
          ],
          const SizedBox(height: 12),
          Row(
          ...
          if (_matchingItems.isNotEmpty) ...[
            ...
            const Text(
              'PALETTE-MATCHED PIECES',
              style: TextStyle(color: AppColors.textMuted, fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1),
            ),
  ...
  Widget _miniChip(String text, Color accent, {bool subtle = false}) => Container(
        ...
        decoration: BoxDecoration(
          color: subtle ? AppColors.surfaceMuted : accent.withValues(alpha: .09),
          borderRadius: BorderRadius.circular(99),
          border: Border.all(
            color: subtle ? AppColors.border : accent.withValues(alpha: .15),
          ),
        ),
        child: Text(
          text,
          style: TextStyle(
            color: subtle ? AppColors.textSecondary : accent,
            fontSize: 9.5,
            fontWeight: FontWeight.w800,
          ),
        ),
  ...
  Widget _contextMetric(String value, String label) => Column(
        children: [
          Text(value, style: const TextStyle(color: AppColors.primary, fontSize: 14, fontWeight: FontWeight.w900)),
          const SizedBox(height: 2),
          Text(label, style: const TextStyle(color: AppColors.textSecondary, fontSize: 8.5)),
        ],
      );
  ...
  Widget _hero(String season, SeasonColourProfile profile, Color accent) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: accent.withValues(alpha: .16), blurRadius: 26, offset: const Offset(0, 12)),
        ],
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
          ...
          if (profile.bestColours.isNotEmpty) ...[
            ...
            SizedBox(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (_, index) => ColourSwatch(name: profile.bestColours[index], size: 44),
              ),
            ),
          ],
        ],
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 7),
                  const Text(
                    'VYEA uses this colour identity across wardrobe matching, makeup guidance and personal styling.',
  ...
  Widget _avoidSection(List<String> colours, Color accent) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 5),
            const Text(
              'Use these shades more carefully when they sit close to your face or dominate an outfit.',
            ...
            children: [
              Expanded(
                child: _sectionHeading(
                  'Colour Psychology',
                  'Use colour intentionally for the impression you want to create.',
                ),
              ),
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: colours.map((name) => ColourSwatch(name: name, size: 40, showLabel: true)).toList(),
            ),
          ],
        ),
      );

  Widget _palette(List<String> colours) => Wrap(
        spacing: 10,
        runSpacing: 11,
        children: colours.map((name) => ColourSwatch(name: name, size: 64, showLabel: true)).toList(),
      );
  ...
  Widget _emptyPalette() => Container(
        ...
        child: const Text(
          'Your personalised palette will appear here after analysis.',
  ...
  Widget _reportActions() => Container(
        ...
        child: Column(
          ...
          children: [
            const Text(
              'SAVE YOUR COLOUR PROFILE',
              style: TextStyle(color: AppColors.textSecondary, fontSize: 10, fontWeight: FontWeight.w800, letterSpacing: 1.1),
            ),
            const SizedBox(height: 10),
            Row(
              children: [
            ...
            const Text(
              'Saved reports are available in Files > On My iPhone > TiB AI Personal Styling > Reports.',
  ...
  Widget _nextStepCard(Color accent) => Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              'NOW STYLE AROUND THIS COLOUR IDENTITY',
              style: TextStyle(color: accent.withValues(alpha: .95), fontSize: 9.5, fontWeight: FontWeight.w800, letterSpacing: 1.05),
            ),
            const SizedBox(height: 7),
            const Text(
              'Turn your result into outfits that still feel like you.',
              style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900, letterSpacing: -.4),
            ),
            const SizedBox(height: 12),
            FilledButton.icon(
              onPressed: _generatingReport ? null : _openAIOutfit,
