...
class _AnalysisResultScreenState extends State<AnalysisResultScreen> {
  ...
  Future<void> _downloadReport() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('PDF saved to Files > On My iPhone > TiB AI Personal Styling > Reports.')),
      );
    } catch (error) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _shareReport() async {
    ...
    try {
      await ColourReportService.generateAndShare(
        result: result,
        shareText: 'My ${result.season} personal colour analysis from VYEA.',
      );
    } catch (error) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
                    ...
                    SliverPadding(
                      ...
                      sliver: SliverList(
                        delegate: SliverChildListDelegate([
                          ...
                          _hero(profile, accent, wide),
                          const SizedBox(height: 18),
                          _attributeRow(accent, wide),
                          const SizedBox(height: 18),
                          _faceShapeCard(accent),
                          if (result.colourReasons.isNotEmpty) ...[const SizedBox(height: 18), _colourReasonCard(accent)],
                          const SizedBox(height: 22),
                          _direction(profile, accent),
                          const SizedBox(height: 22),
                          _personalContextCard(accent, wide),
                          ...
                          if (result.faceStylingGuidance.isNotEmpty) ...[const SizedBox(height: 24), _faceGuidanceCard(accent)],
                          if (result.imageUrl.isNotEmpty) ...[
                            ...
                            ClipRRect(
                              ...
                              child: CachedNetworkImage(
                                ...
                                fit: BoxFit.cover,
                                placeholder: (_, __) => Container(height: wide ? 380 : 250, color: AppColors.surfaceMuted, child: const Center(child: CircularProgressIndicator())),
                                errorWidget: (_, __, ___) => Container(height: wide ? 380 : 250, color: AppColors.surfaceMuted, child: const Center(child: Icon(Icons.image_not_supported_outlined))),
                              ),
                            ),
  ...
  Widget _hero(SeasonColourProfile profile, Color accent, bool wide) => Container(
  ...
  Widget _attributeRow(Color accent, bool wide) => GridView.count(
    shrinkWrap: true,
    physics: const NeverScrollableScrollPhysics(),
    crossAxisCount: wide ? 3 : 1,
    crossAxisSpacing: 8,
    mainAxisSpacing: 8,
    childAspectRatio: wide ? 2.9 : 3.6,
    children: [
      _metric('UNDERTONE', result.undertone, accent),
      _metric('DEPTH', result.brightness, accent),
      _metric('CONTRAST', result.contrast, accent),
    ],
  );

  Widget _faceShapeCard(Color accent) => Container(
    ...
    child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 4),
        Text((result.faceShape.trim().isEmpty ? 'Unknown' : result.faceShape).toUpperCase(), style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
        if (result.faceShapeDescription.isNotEmpty) ...[const SizedBox(height: 4), Text(result.faceShapeDescription, style: const TextStyle(color: AppColors.textSecondary, fontSize: 11.5, height: 1.35))],
      ])),
    ]),
  );

  Widget _colourReasonCard(Color accent) => Container(
  ...
  Widget _faceGuidanceCard(Color accent) => Container(
    padding: const EdgeInsets.fromLTRB(17, 17, 17, 15),
    decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(22), border: Border.all(color: AppColors.border)),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [Icon(Icons.auto_awesome_outlined, color: accent, size: 20), const SizedBox(width: 8), const Expanded(child: Text('STYLE FROM YOUR FACE SHAPE', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: .9)))]),
  ...
  Widget _metric(String label, String value, Color accent) => Container(
    ...
    child: Row(children: [Container(width: 7, height: 7, decoration: BoxDecoration(color: accent, shape: BoxShape.circle)), const SizedBox(width: 9), Expanded(child: Text(label, style: const TextStyle(color: AppColors.textMuted, fontSize: 8.5, fontWeight: FontWeight.w900, letterSpacing: .9))), Flexible(child: Text(value.isEmpty ? 'Unknown' : value, textAlign: TextAlign.right, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.w800))) ]),
  );

  Widget _personalContextCard(Color accent, bool wide) {
    ...
    final metrics = [
      _contextMetric('$_wardrobeCount', 'wardrobe'),
      _contextMetric('$_favourites', 'favourites'),
      _contextMetric('$_savedLooks', 'saved looks'),
    ];
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 12),
        if (wide) Row(children: [for (var i = 0; i < metrics.length; i++) ...[Expanded(child: metrics[i]), if (i < metrics.length - 1) _contextDivider()]]) else Column(children: [for (var i = 0; i < metrics.length; i++) ...[metrics[i], if (i < metrics.length - 1) const Divider(height: 16)]]),
        const SizedBox(height: 12),
        wide ? Row(children: [Expanded(child: OutlinedButton.icon(onPressed: _openStyle, icon: const Icon(Icons.tune_rounded, size: 16), label: const Text('Refine Style'))), const SizedBox(width: 8), Expanded(child: FilledButton.icon(onPressed: _openAIStylist, icon: const Icon(Icons.auto_awesome_rounded, size: 16), label: const Text('Style Me')))]) : Column(children: [SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: _openStyle, icon: const Icon(Icons.tune_rounded, size: 16), label: const Text('Refine Style'))), const SizedBox(height: 8), SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: _openAIStylist, icon: const Icon(Icons.auto_awesome_rounded, size: 16), label: const Text('Style Me')))]),
      ]),
    );
  }

  Widget _direction(SeasonColourProfile profile, Color accent) => Container(padding: const EdgeInsets.all(17), decoration: BoxDecoration(color: accent.withValues(alpha: .07), borderRadius: BorderRadius.circular(21), border: Border.all(color: accent.withValues(alpha: .15))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('YOUR COLOUR DIRECTION', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1)), const SizedBox(height: 7), Text(profile.dimension, style: TextStyle(color: accent, fontSize: 15, fontWeight: FontWeight.w900)), const SizedBox(height: 5), const Text('Use these characteristics as a starting point across clothing, makeup and accessories.', style: TextStyle(color: AppColors.textSecondary, fontSize: 11.5, height: 1.4))]));
  ...
  Widget _makeupSection(String title, List<String> names, Color accent) => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(21), border: Border.all(color: AppColors.border)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800)), const SizedBox(height: 10), Text(names.isEmpty ? 'No guidance available yet.' : names.join(' · '), style: const TextStyle(color: AppColors.textSecondary, fontSize: 11.5, height: 1.4))]));

  Widget _reportActions() => Row(children: [Expanded(child: OutlinedButton.icon(onPressed: _generatingReport ? null : _downloadReport, icon: const Icon(Icons.download_outlined, size: 17), label: const Text('PDF'))), const SizedBox(width: 8), Expanded(child: FilledButton.icon(onPressed: _generatingReport ? null : _shareReport, icon: const Icon(Icons.ios_share_outlined, size: 17), label: const Text('Share')))]);

  Widget _nextStepCard(Color accent) => _actionCard(accent: accent, icon: Icons.arrow_forward_rounded, title: 'Ready for the next step?', description: 'Use this result to refine your wardrobe and ask VYEA for an outfit that fits the way you want to dress.', button: 'Open AI Stylist', onPressed: _openAIStylist);

  Widget _actionCard({required Color accent, required IconData icon, required String title, required String description, required String button, required VoidCallback onPressed}) => Container(padding: const EdgeInsets.all(17), decoration: BoxDecoration(color: accent.withValues(alpha: .07), borderRadius: BorderRadius.circular(21), border: Border.all(color: accent.withValues(alpha: .16))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(icon, color: accent, size: 22), const SizedBox(height: 9), Text(title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900)), const SizedBox(height: 5), Text(description, style: const TextStyle(color: AppColors.textSecondary, fontSize: 11.5, height: 1.4)), const SizedBox(height: 12), SizedBox(width: double.infinity, child: FilledButton(onPressed: onPressed, child: Text(button)))]));

  Widget _miniChip(String label, Color accent, {bool subtle = false}) => Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: subtle ? AppColors.surfaceMuted : accent.withValues(alpha: .08), borderRadius: BorderRadius.circular(99)), child: Text(label, style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700, color: subtle ? AppColors.textSecondary : accent)));
  Widget _contextMetric(String value, String label) => Container(padding: const EdgeInsets.symmetric(vertical: 4), child: Column(children: [Text(value, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900)), const SizedBox(height: 2), Text(label, style: const TextStyle(color: AppColors.textSecondary, fontSize: 9.5))]));
  Widget _contextDivider() => Container(width: 1, height: 30, color: AppColors.border);
}
