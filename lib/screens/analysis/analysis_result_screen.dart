...
class _AnalysisResultScreenState extends State<AnalysisResultScreen> {
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Your style identity', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
            SizedBox(height: 3),
            Text('Your colour result is now connected to the rest of your VYEA profile.', style: TextStyle(color: AppColors.textSecondary, fontSize: 11.5, height: 1.35)),
          ])),
        ]),
        const SizedBox(height: 12),
  ...
  Widget _hero(String season, SeasonColourProfile profile, Color accent) => Container(
  ...
  Widget _metric(String label, String value, Color accent) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(label, style: const TextStyle(color: AppColors.textMuted, fontSize: 8, fontWeight: FontWeight.w900, letterSpacing: .7)),
          const SizedBox(height: 5),
  ...
  Widget _direction(SeasonColourProfile profile, Color accent) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('YOUR COLOUR DIRECTION', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1)),
          const SizedBox(height: 7),
          Text(profile.dimension, style: TextStyle(color: accent, fontSize: 15, fontWeight: FontWeight.w900)),
          const SizedBox(height: 5),
          Text('Use these characteristics as a starting point across clothing, makeup and accessories.', style: const TextStyle(color: AppColors.textSecondary, fontSize: 11.5, height: 1.4)),
        ]),
      );

  Widget _sectionHeading(String title, String subtitle) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget _avoidSection(List<String> avoid, Color accent) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('COLOURS TO APPROACH WITH CARE', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1)),
          const SizedBox(height: 8),
          Text(avoid.isEmpty ? 'Use personal preference and mirror testing.' : avoid.join(' · '), style: const TextStyle(color: AppColors.textSecondary, fontSize: 11.5, height: 1.4)),
        ]),
      );
  ...
  Widget _colourPsychologyPreview(Color accent) => Container(
        ...
        child: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('COLOUR PERSONALITY', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1)),
          SizedBox(height: 7),
          Text('Your palette can help guide the mood and visual energy you want to express.', style: TextStyle(color: AppColors.textSecondary, fontSize: 11.5, height: 1.4)),
        ]),
      );
  ...
  Widget _makeupSection(String title, List<String> names, Color accent) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 10),
          Text(names.join(' · '), style: const TextStyle(color: AppColors.textSecondary, fontSize: 11.5, height: 1.4)),
        ]),
      );

  Widget _reportActions() => Row(children: [
        Expanded(child: OutlinedButton.icon(onPressed: _generatingReport ? null : _downloadReport, icon: const Icon(Icons.download_outlined, size: 17), label: const Text('PDF'))),
        const SizedBox(width: 8),
        Expanded(child: FilledButton.icon(onPressed: _generatingReport ? null : _shareReport, icon: const Icon(Icons.ios_share_rounded, size: 17), label: const Text('Share'))),
      ]);

  Widget _nextStepCard(Color accent) => _actionCard(
        accent: accent,
        icon: Icons.auto_awesome_rounded,
        title: 'Take your result into your wardrobe',
        description: 'Use your personal colours and face shape to make your next outfit decisions easier.',
  ...
  Widget _actionCard({required Color accent, required IconData icon, required String title, required String description, required String button, required VoidCallback onPressed}) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 4),
          Text(description, style: const TextStyle(color: AppColors.textSecondary, fontSize: 11.5, height: 1.4)),
          const SizedBox(height: 12),
  ...
  Widget _miniChip(String text, Color accent, {bool subtle = false}) => Container(
        ...
        decoration: BoxDecoration(color: subtle ? AppColors.surfaceMuted : accent.withValues(alpha: .08), borderRadius: BorderRadius.circular(10), border: Border.all(color: subtle ? AppColors.border : accent.withValues(alpha: .18))),
        child: Text(text, style: const TextStyle(fontSize: 9.5, fontWeight: FontWeight.w700)),
      );

  Widget _contextMetric(String value, String label) => Column(children: [
