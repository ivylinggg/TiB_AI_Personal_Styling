...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> loadUser() async {
    ...
    try {
      ...
      loadedUser ??= preferenceData.user;
      loadedAnalysis ??= preferenceData.colourAnalysis;
    } catch (_) {}
  ...
  Widget identityHero() {
    ...
    return Container(
      ...
      child: Column(children: [
        Row(children: [
          CircleAvatar(radius: 39, backgroundColor: AppColors.secondary, backgroundImage: photoUrl.isNotEmpty ? CachedNetworkImageProvider(photoUrl) : null, child: photoUrl.isNotEmpty ? null : const Icon(Icons.person_outline_rounded, size: 36, color: AppColors.primaryDark)),
  ...
  Widget identityPanel(ColourAnalysisResult? result) {
    if (result == null) {
      return Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(gradient: AppGradients.primary, borderRadius: BorderRadius.circular(AppRadius.xl)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Build your colour identity', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900)), const SizedBox(height: 10), SizedBox(width: double.infinity, child: FilledButton(onPressed: isPreview ? null : openColourAnalysis, child: Text(isPreview ? 'Preview only' : 'Start Colour Analysis')))]));
    }
    return Container(width: double.infinity, padding: const EdgeInsets.all(18), decoration: BoxDecoration(gradient: AppGradients.season(result.season), borderRadius: BorderRadius.circular(AppRadius.xl)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(result.season, style: const TextStyle(color: Colors.white, fontSize: 29, fontWeight: FontWeight.w900)), const SizedBox(height: 5), Text('${result.undertone} • ${result.brightness} • ${result.contrast}', style: const TextStyle(color: Colors.white70)), if (styles.isNotEmpty) ...[const SizedBox(height: 12), Wrap(spacing: 6, runSpacing: 6, children: styles.take(3).map(_lightStyleTag).toList())]]));
  }
