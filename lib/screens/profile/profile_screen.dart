...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  bool get _isPreview => context.read<PreviewContext>().isCustomerPreview;
  ...
  Future<void> openEditProfile() async {
    ...
    if (firebaseUser == null || user == null || _isPreview) return;
  ...
  Future<void> changePassword() async {
    ...
    if (email == null || _isPreview) return;
  ...
  Widget _buildBody(ColourAnalysisResult? analysisResult) {
    ...
    if (_activeUid == null) return const Center(child: Text('Please login to view your profile.'));
    if (user == null) {
      return Padding(
  ...
  Widget _buildIdentityHero() {
    ...
    final displayEmail = user?.email.trim().isNotEmpty == true ? user!.email.trim() : (FirebaseAuth.instance.currentUser?.email ?? '');
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        Container(padding: const EdgeInsets.symmetric(horizontal: 13, vertical: 11), decoration: BoxDecoration(color: AppColors.primaryDark, borderRadius: BorderRadius.circular(17)), child: Row(children: [const Icon(Icons.auto_awesome_rounded, size: 17, color: AppColors.peach), const SizedBox(width: 8), Expanded(child: Text('$styleCount styles · $preferenceCount preferences saved', style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w700))), const SizedBox(width: 8), Text(isPremium ? 'PERSONAL+' : 'PERSONAL', style: const TextStyle(color: Colors.white70, fontSize: 8.5, fontWeight: FontWeight.w900, letterSpacing: .9))])),
        if (!_isPreview) ...[const SizedBox(height: 13), SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: openEditProfile, icon: const Icon(Icons.edit_outlined, size: 16), label: const Text('Edit Profile'))],
      ]),
    );
  }
