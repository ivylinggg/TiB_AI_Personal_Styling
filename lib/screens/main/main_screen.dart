...
class _MainScreenState extends State<MainScreen> {
  ...
  String? get _previewUid => context.read<PreviewContext>().customerUid;
  String? get _dataUid => widget.adminPreview ? _previewUid : _uid;
  ...
  void _returnToAdmin() {
    ...
    Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (_) => const AdminMainScreen()), (_) => false);
  }
            ...
            builder: (context, snapshot) {
              ...
              return Padding(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [const Expanded(child: Text('Notifications', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900))), if (unreadCount > 0) TextButton(onPressed: () => NotificationService.markAllRead(uid), child: Text('Mark all read ($unreadCount)'))]),
                  ...
                  if (snapshot.connectionState == ConnectionState.waiting && !snapshot.hasData)
                    ...
                    Expanded(child: ListView.separated(itemCount: notifications.length, separatorBuilder: (_, _) => const SizedBox(height: 8), itemBuilder: (context, index) {
  ...
  Widget _notificationBell() {
    final uid = _uid;
    if (uid == null) return _topActionButton(icon: Icons.notifications_none_rounded, tooltip: 'Notifications', onTap: _showNotifications);
  ...
  Future<void> _logout() async {
    ...
    final confirmed = await showDialog<bool>(context: context, builder: (dialogContext) => AlertDialog(title: const Text('Log out?'), content: const Text('You can sign back in anytime to continue your styling journey.'), actions: [TextButton(onPressed: () => Navigator.pop(dialogContext, false), child: const Text('Cancel')), FilledButton(onPressed: () => Navigator.pop(dialogContext, true), child: const Text('Log out'))]));
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          if (widget.adminPreview) ...[_topActionButton(icon: Icons.admin_panel_settings_outlined, tooltip: 'Return to Admin', onTap: _returnToAdmin), const SizedBox(width: 6)],
          _notificationBell(), const SizedBox(width: 6), _topActionButton(icon: Icons.logout_rounded, tooltip: 'Log out', onTap: _logout),
        ]),
      ),
    );
  }

  Widget _topActionButton({required IconData icon, required String tooltip, required VoidCallback onTap}) => Tooltip(message: tooltip, child: Material(color: AppColors.surfaceMuted, shape: const CircleBorder(), child: InkWell(onTap: onTap, customBorder: const CircleBorder(), child: SizedBox(width: 39, height: 39, child: Icon(icon, color: AppColors.primary, size: 18)))));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: SafeArea(top: false, minimum: const EdgeInsets.fromLTRB(12, 0, 12, 10), child: Container(
        ...
        child: ClipRRect(borderRadius: BorderRadius.circular(22), child: NavigationBar(selectedIndex: _selectedIndex, backgroundColor: Colors.transparent, elevation: 0, height: 70, labelBehavior: NavigationDestinationLabelBehavior.alwaysShow, indicatorColor: AppColors.primarySoft, onDestinationSelected: _selectTab, destinations: [
