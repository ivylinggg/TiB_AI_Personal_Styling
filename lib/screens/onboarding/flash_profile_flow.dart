...
class _FlashProfileFlowState extends State<FlashProfileFlow> {
  ...
  Widget _selectCard(String label, {required String emoji, required bool selected, required VoidCallback onTap}) => InkWell(
        ...
        children: [
          ...
          Expanded(
            child: Container(
              ...
              decoration: BoxDecoration(borderRadius: BorderRadius.circular(28), gradient: const LinearGradient(colors: [Color(0xFFF1E8FA), Color(0xFFFFF1EA)], begin: Alignment.topLeft, end: Alignment.bottomRight)),
              child: _scanImage == null
                  ? const Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Icon(Icons.face_retouching_natural_outlined, color: AppColors.primary, size: 58),
                      SizedBox(height: 18),
                      Text('Upload a clear face photo', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800)),
                      SizedBox(height: 6),
                      Text('Good lighting works best.', style: TextStyle(color: AppColors.textSecondary, fontSize: 12)),
                    ])
                  : ClipRRect(borderRadius: BorderRadius.circular(28), child: Image.file(_scanImage!, width: double.infinity, height: double.infinity, fit: BoxFit.cover)),
            ),
          ),
          const SizedBox(height: 12),
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            OutlinedButton.icon(onPressed: _saving ? null : _openFaceScan, icon: const Icon(Icons.camera_alt_outlined, size: 17), label: const Text('Scan')),
            const SizedBox(width: 10),
            OutlinedButton.icon(onPressed: _saving ? null : _pickPhoto, icon: const Icon(Icons.photo_library_outlined, size: 17), label: const Text('Gallery')),
          ]),
        ],
      );
}
